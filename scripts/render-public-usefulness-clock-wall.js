...
const list = (items, empty = 'No item recorded in this build.') => {
  const values = (items || []).map(item => clean(item, 1200)).filter(Boolean);
...
const descriptions = {
  ...
  'Speculative Watch': 'Classified claims from the Dark Speculation Lab. Scores measure evidence pressure around a claim, never truth, guilt or event probability.'
};
const speculationSectionOrder = [
  'Control System Convergence',
  'Elite, Blackmail and Ritual Claims',
  'Metaphysics and Soul-Prison Claims',
  'Aliens, UAP and Hidden Bases',
  'Mind Control, Media and Psyops',
  'Documentable Exploitation and Trafficking',
  'Medical and Bio Allegations',
  'Technology, Religion and Smart Cities',
  'Hidden History and Cataclysm Claims',
  'Other Classified Claims'
];

function speculation(clock) {
  if (clock.speculationOnly) {
    return `${clock.researchBrief || 'Test the claim against original records and counter-sources.'} Support standard: ${clock.supportStandard || 'Primary evidence is required.'} This remains a classified research question, not a finding.`;
  }
...
function evidenceSources(clock) {
  return (clock.evidenceInputs || []).slice(0, 8).map(item => {
...
function card(clock) {
  ...
  const claimBadges = clock.speculationOnly
    ? `<span class="clock-claim-class">${escapeHtml(clock.claimClass || 'classified claim')}</span><span class="clock-claim-group">${escapeHtml(clock.speculationGroup || 'Speculation')}</span>`
    : '';
  const claimSection = clock.speculationOnly ? `<section class="claim-classification">
        <h4>Claim classification</h4>
        <p><strong>Class:</strong> ${escapeHtml(clock.claimClass || '')}</p>
        <p><strong>Source lane:</strong> ${escapeHtml(clock.speculationGroup || '')}</p>
        <p><strong>Risk rating:</strong> ${escapeHtml(clock.riskRating || '')}</p>
        <p><strong>Research brief:</strong> ${escapeHtml(clock.researchBrief || '')}</p>
        <p><strong>Support standard:</strong> ${escapeHtml(clock.supportStandard || '')}</p>
        <p><strong>Falsification test:</strong> ${escapeHtml(clock.falsificationTest || '')}</p>
        <p><strong>Automatic evidence gate:</strong> ${escapeHtml(clock.evidenceGate || '')}</p>
      </section>` : '';
  return `<article class="clock-card${clock.speculationOnly ? ' speculation-clock' : ''}" id="${escapeHtml(clock.slug || '')}" data-clock-score="${score}" data-clock-category="${escapeHtml(clock.category || legacyCategories[clock.slug] || 'Global Watch')}"${clock.speculationOnly ? ` data-claim-class="${escapeHtml(clock.claimClass || '')}" data-speculation-section="${escapeHtml(clock.speculationSection || '')}"` : ''}>
    <div class="clock-topline" data-clock-summary-only="true"><span class="clock-score-badge">${score}%</span><span class="clock-window">${escapeHtml(clock.window || 'Review window not set')}</span>${claimBadges}</div>
    ...
    <details class="clock-detail"><summary>Open deeper information</summary><div class="clock-detail-body">
      ...
      ${claimSection}
      <section class="speculation-panel"><h4>Speculation angle</h4><p><strong>Research trajectory:</strong> ${escapeHtml(speculation(clock))}</p></section>
      ...
      <section><h4>Evidence feeding this timer</h4><ul>${evidenceSources(clock)}</ul></section>
...
function regularGrid(clocks) {
  return `<div class="timer-grid">${clocks.map(card).join('')}</div>`;
}
function speculationBody(clocks) {
  const existing = clocks.filter(clock => !clock.speculationOnly);
  const classified = clocks.filter(clock => clock.speculationOnly);
  const groups = new Map();
  for (const clock of classified) {
    const section = clock.speculationSection || 'Other Classified Claims';
    if (!groups.has(section)) groups.set(section, []);
    groups.get(section).push(clock);
  }
  const sectionNav = speculationSectionOrder.filter(section => groups.get(section)?.length)
    .map(section => `<a href="#speculation-section-${section.toLowerCase().replace(/[^a-z0-9]+/g, '-')}">${escapeHtml(section)}</a>`).join('');
  const sections = speculationSectionOrder.filter(section => groups.get(section)?.length).map((section, index) => {
    const items = groups.get(section);
    const classSummary = [...new Set(items.map(item => item.claimClass).filter(Boolean))].join(' · ');
    return `<details class="speculation-section" id="speculation-section-${section.toLowerCase().replace(/[^a-z0-9]+/g, '-')}"${index === 0 ? ' open' : ''}><summary><strong>${escapeHtml(section)}</strong><span>${items.length} clocks</span></summary><p class="speculation-section-meta">${escapeHtml(classSummary)}</p>${regularGrid(items)}</details>`;
  }).join('');
  return `<article class="speculation-boundary"><h3>Classified claims, not confirmed events</h3><p>These clocks are generated from the Dark Speculation Lab registry. Their percentages measure source-linked evidence pressure around a claim. They do not measure truth, guilt or probability. Internet mythology, paranormal claims and unsupported extreme allegations cannot rise automatically from mentions, symbols, screenshots or repetition.</p><div class="cta-row small"><a class="btn alt" href="dark-speculation-lab.html">Open Dark Speculation Lab</a><a class="btn alt" href="claim-classifier.html">Open Claim Classifier</a><a class="btn alt" href="dark-speculation-forum.html">Drop A Counter-Source</a></div></article>${existing.length ? `<h3>Existing disclosure watch</h3>${regularGrid(existing)}` : ''}<nav class="speculation-section-nav">${sectionNav}</nav>${sections}`;
}
...
const sections = order.filter(category => grouped.get(category)?.length).map(category => `<section class="section wrap clock-category" id="clock-group-${category.toLowerCase().replace(/[^a-z0-9]+/g, '-')}"><div class="clock-category-heading"><div><div class="eyebrow">Reader early-warning dashboard</div><h2>${escapeHtml(category)}</h2><p>${escapeHtml(descriptions[category])}</p></div><span>${grouped.get(category).length} clocks</span></div>${category === 'Speculative Watch' ? speculationBody(grouped.get(category)) : regularGrid(grouped.get(category))}</section>`).join('');
const pressure = standard?.scoreTypes?.pressureIndex || {};
const html = `<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"><title>Mission Timers | Matrix Reprogrammed</title><meta name="description" content="Evidence-fed early-warning clocks for freedom, household security, essential services, accountable government, global risks and separately classified speculation."><link rel="stylesheet" href="styles.css"><link rel="stylesheet" href="fixes.css"><link rel="stylesheet" href="reader-experience.css"><style>
.timer-hero{padding:2rem 1rem}.timer-hero-box{border:1px solid rgba(216,181,106,.35);border-radius:28px;padding:2rem;background:radial-gradient(circle at 30% 0,rgba(180,0,0,.25),transparent 38%),linear-gradient(135deg,rgba(12,0,0,.96),rgba(0,0,0,.94))}.timer-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(330px,1fr));gap:1rem;align-items:start}.clock-card{border:1px solid rgba(216,181,106,.25);border-radius:22px;padding:1.1rem;background:linear-gradient(150deg,rgba(12,12,12,.97),rgba(28,0,0,.72));box-shadow:0 18px 55px rgba(0,0,0,.28)}.speculation-clock{border-color:rgba(190,55,55,.38);background:linear-gradient(150deg,rgba(15,8,8,.98),rgba(38,0,0,.78))}.clock-topline{display:flex;gap:.45rem;flex-wrap:wrap;margin-bottom:.85rem}.clock-score-badge,.clock-window,.clock-band,.clock-category-nav a,.clock-category-heading>span,.clock-claim-class,.clock-claim-group,.speculation-section-nav a{border:1px solid rgba(216,181,106,.35);border-radius:999px;padding:.3rem .58rem;font-size:.82rem}.clock-claim-class{border-color:rgba(190,55,55,.65)}.clock-claim-group{opacity:.82}.clock-score-badge{font-weight:900;color:#f0d28b}.clock-summary{display:grid;grid-template-columns:112px 1fr;gap:1rem;align-items:center}.clock-ring{width:112px;height:112px;border-radius:50%;display:grid;place-items:center;background:conic-gradient(rgba(210,40,35,.95) calc(var(--p)*1%),rgba(255,255,255,.08) 0)}.clock-ring strong{font-size:1.55rem}.clock-heading h3{margin:.55rem 0}.clock-change{margin:0;color:#eee0bb}.clock-band{display:inline-block}.clock-detail{margin-top:1rem;border-top:1px solid rgba(216,181,106,.22);padding-top:.85rem}.clock-detail summary{cursor:pointer;font-weight:800;color:#f0d28b;list-style-position:inside}.clock-detail-body{display:grid;gap:.9rem;padding-top:.9rem}.clock-detail-body section,.clock-boundary,.speculation-boundary{padding:.85rem;border:1px solid rgba(216,181,106,.16);border-radius:12px;background:rgba(255,255,255,.025)}.claim-classification,.speculation-panel{border-color:rgba(190,55,55,.5)!important;background:rgba(120,0,0,.12)!important}.clock-columns{display:grid!important;grid-template-columns:1fr 1fr;gap:.8rem}.clock-links,.clock-category-nav,.speculation-section-nav{display:flex;gap:.45rem;flex-wrap:wrap}.clock-links a{border:1px solid rgba(216,181,106,.25);border-radius:999px;padding:.3rem .55rem}.clock-category{scroll-margin-top:1rem}.clock-category-heading{display:flex;justify-content:space-between;align-items:end;gap:1rem;margin-bottom:1rem}.clock-category-heading h2{margin:.25rem 0}.clock-category-heading p{margin:0;max-width:760px}.speculation-boundary{margin-bottom:1rem;border-color:rgba(190,55,55,.55);background:rgba(70,0,0,.18)}.speculation-section-nav{margin:1rem 0}.speculation-section{border:1px solid rgba(216,181,106,.24);border-radius:16px;padding:.8rem;margin:1rem 0;scroll-margin-top:1rem}.speculation-section>summary{display:flex;justify-content:space-between;gap:1rem;cursor:pointer;color:#f0d28b}.speculation-section-meta{opacity:.76;margin:.6rem 0 1rem}@media(max-width:680px){.clock-summary{grid-template-columns:1fr}.clock-ring{margin:auto}.clock-columns{grid-template-columns:1fr!important}.clock-category-heading{align-items:start;flex-direction:column}.speculation-section>summary{flex-direction:column}}
</style></head><body><canvas id="matrix"></canvas><div class="signal-face"></div><div class="veil"></div><div class="page"><header class="wrap topbar"><a class="brand" href="index.html"><img src="sigil.png" alt="Matrix Reprogrammed sigil"> MATRIX REPROGRAMMED</a><nav class="nav"><a href="daily-command-brief.html">Daily Brief</a><a href="control-system-tracker.html">Control Tracker</a><a href="evidence-vault.html">Evidence</a><a href="search.html">Search</a></nav></header><main><section class="timer-hero wrap"><div class="timer-hero-box"><div class="eyebrow">Evidence synthesis · updated ${escapeHtml(wall.updated || '')}</div><h1>MISSION TIMERS.</h1><p class="lead">Each collapsed card shows only its percentage, timeframe, pressure band, title and latest movement. Open deeper information for evidence, calculation, speculation, counter-signals, missing records and source routes.</p><p><strong>Important:</strong> these percentages are pressure indexes, not predictions that an event has the same percentage chance of occurring. Speculation clocks are additionally classified and evidence-gated.</p><p><strong>Method:</strong> ${escapeHtml(pressure.method || '')}</p><div class="cta-row"><a class="btn" href="data/clock-wall.json">Open Machine Data</a><a class="btn alt" href="downloads/timer-synthesis.md">Download Synthesis</a><a class="btn alt" href="evidence-vault.html">Verify Evidence</a><a class="btn alt" href="dark-speculation-lab.html">Dark Speculation Lab</a></div></div></section><section class="section wrap"><div class="eyebrow">Choose what affects you</div><h2>Reader early-warning dashboard</h2><p>Start with freedom, money, essential services or government. Global and speculative watches remain clearly separated.</p><nav class="clock-category-nav">${nav}</nav></section>${sections}</main><footer class="footer wrap"><p><strong>MATRIX REPROGRAMMED</strong> — source first, claim second, usefulness always.</p></footer></div><script src="matrix.js"></script><script src="analytics.js"></script></body></html>`;
fs.writeFileSync(path.join(root, 'timers.html'), html);
console.log(`Final enriched clock wall rendered: ${(wall.clocks || []).length} clocks across ${order.filter(category => grouped.get(category)?.length).length} categories, including ${wall.speculativeClockCount || 0} classified speculation clocks.`);
