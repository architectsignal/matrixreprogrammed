...
function relatedCards(book) {
  ...
  return `<section class="section wrap"><h2>Related Doors</h2><div class="grid">${related.map(b => `<article class="card"><span class="label">${esc(b.category)}</span><h3>${esc(b.title)}</h3><p>${esc(b.description)}</p><a class="btn" href="${esc(b.generatedUrl || b.localUrl)}">Open Door</a></article>`).join('')}</div></section>`;
}

for (const book of books) {
  const schema = `<script type="application/ld+json">${JSON.stringify({ '@context': 'https://schema.org', '@type': 'Book', name: book.title, author: { '@type': 'Person', name: 'Nicholas Matthews' }, publisher: { '@type': 'Organization', name: 'Matrix Reprogrammed' }, description: book.description, url: `https://matrixreprogrammed.com/${book.generatedUrl || book.localUrl}` })}</script>`;
...
const searchIndex = books.map(book => ({ key: book.key, title: book.title, subtitle: book.subtitle, series: book.series, category: book.category, url: book.generatedUrl || book.localUrl, description: book.description, keywords: book.keywords || [] }));
...
const startBody = `<main><section class="hero wrap"><div class="eyebrow">Start Here</div><h1>CHOOSE YOUR DOOR.</h1><p class="lead">The archive is large by design. Pick the signal you are trying to understand and enter through the right reader path.</p><div class="cta-row"><a class="btn" href="black-file.html">Open The Black File</a><a class="btn alt" href="search.html">Search Archive</a></div></section><section class="section wrap grid">${startCards.map(([title, desc, keys]) => `<article class="card"><h2>${esc(title)}</h2><p>${esc(desc)}</p>${keys.map(key => books.find(b => b.key === key)).filter(Boolean).map(b => `<p><a class="btn" href="${esc(b.generatedUrl || b.localUrl)}">${esc(b.title)}</a></p>`).join('')}</article>`).join('')}</section></main>`;
...
console.log(`Built ${books.length} generated book pages, start-here.html, search.html, and search-index.json.`);
