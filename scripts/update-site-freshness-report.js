...
const reportVersion = 'site-freshness-v2';
const htmlFiles = [];
function walk(dir) {
...
function isObsoleteMembershipFigure(figure) { return /€\s*(?:19|49)\s*(?:\/|per\s*)month/i.test(`${figure.value} ${figure.context}`); }
const pages = htmlFiles.map(file => {
  const html = fs.readFileSync(path.join(root, file), 'utf8');
  const copy = visibleCopy(html);
  const percentages = contexts(copy, /\b\d+(?:\.\d+)?\s*%\b/g);
  const money = contexts(copy, /(?:[€$£]\s?\d[\d,.]*|\b\d[\d,.]*\s?(?:million|billion|trillion)\b)/gi).filter(figure => !isObsoleteMembershipFigure(figure));
...
if (/€\s*(?:19|49)\s*(?:\/|per\s*)month/i.test(generated)) throw new Error('Site freshness report republished obsolete membership pricing');
