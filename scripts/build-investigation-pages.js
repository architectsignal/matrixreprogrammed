...
function prettyDate(value) {
  ...
  if (!Number.isFinite(date.getTime())) return 'Not yet recorded';
...
function productSection(product, heading) {
  ...
  return `<section class="hero wrap"><div class="eyebrow">${esc(product.kind || 'Investigation')} Intelligence · ${esc(prettyDate(product.generatedAt))}</div><h1>${esc(heading)}</h1><p class="lead">${esc(product.topConclusion || 'No current conclusion loaded.')}</p><p><strong>Boundary:</strong> ${esc(product.boundary || '')}</p><div class="cta-row"><a class="btn" href="investigation-machine.html">Investigation Machine</a><a class="btn alt" href="investigation-source-ledger.html">Source Ledger</a><a class="btn alt" href="search.html">Search Evidence</a></div></section><section class="section wrap split"><div class="terminal">INVESTIGATION STATUS\n&gt; Sources scheduled: ${esc(product.summary?.sourcesScheduled || 0)}\n&gt; Sources fetched: ${esc(product.summary?.sourcesFetched || 0)}\n&gt; Changed sources: ${esc(product.summary?.changedSources || 0)}\n&gt; Findings in window: ${esc(product.summary?.findingsInWindow || 0)}\n&gt; Established wrongdoing: ${esc(product.summary?.establishedWrongdoingFindings || 0)}\n&gt; Official actions: ${esc(product.summary?.officialActionFindings || 0)}\n&gt; Leak/document leads: ${esc(product.summary?.documentOrLeakLeads || 0)}</div><aside class="card redline"><h2>Method</h2><p>The machine ranks official adjudication, enforcement, audit findings, charges, source changes and document releases separately. It connects the record to money, office, contract, authority, implementation and missing evidence.</p></aside></section><section class="section wrap"><h2>Strongest Findings</h2><div class="grid">${findings.length ? findings.map(findingCard).join('') : '<article class="card"><h3>No new finding crossed the threshold</h3><p>This is a neutral result. The source ledger still records every attempted search and failure.</p></article>'}</div></section><section class="section wrap"><h2>Missing Records And Next Moves</h2><div class="grid">${(product.missingRecords || []).slice(0, 20).map(item => `<article class="card"><h3>${esc(item.title)}</h3>${list(item.nextRecords)}<a class="btn alt" href="${esc(item.source)}" rel="noopener">Open Starting Record</a></article>`).join('') || '<article class="card"><p>No missing-record queue generated yet.</p></article>'}</div></section>`;
}
function markdownProduct(product, heading) {
  const lines = [`# ${heading}`, '', `Generated: ${product.generatedAt || 'not yet'}`, '', product.topConclusion || '', '', `Boundary: ${product.boundary || ''}`, '', '## Summary', ''];
