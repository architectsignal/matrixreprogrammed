...
const actors=(synthesis?.inferenceLayer?.actorMap||[]).filter(actor=>{
  const name=clean(actor?.name,180);
  const action=clean(actor?.action||actor?.documentedRole,220);
  const why=clean(actor?.whyItMatters||actor?.summaries?.[0],520);
  const evidenceRoute=clean(actor?.sourceRoute,800);
  return name&&!genericActor.test(name)&&!placeholderActor.test(name)&&action&&why&&evidenceRoute;
}).slice(0,8);
if(!actors.length) throw new Error('Homepage actor intelligence failed: no sufficiently specific named actor with action, significance and evidence route');
const scenarios=(synthesis?.speculativeLayer?.scenarios||[]).slice(0,4);
const actorCards=actors.map(actor=>{
  const action=clean(actor.action||actor.documentedRole,220);
  const authority=clean(actor.authority||'',220);
  const instrument=clean(actor.instrument||'',260);
  const why=clean(actor.whyItMatters||actor.summaries?.[0],520);
  const evidenceRoute=route(actor.sourceRoute);
  return `<article class="actor-intel-card"><h4>${escapeHtml(actor.name)}</h4><p class="actor-action"><strong>Documented action or role:</strong> ${escapeHtml(action)}</p>${authority?`<p><strong>Institution / authority:</strong> ${escapeHtml(authority)}</p>`:''}${instrument?`<p><strong>Law, filing or instrument:</strong> ${escapeHtml(instrument)}</p>`:''}<p><strong>Why it matters:</strong> ${escapeHtml(why)}</p><p class="home-boundary"><strong>Evidence boundary:</strong> ${escapeHtml(actor.boundary||'The cited record establishes the documented role or action only. It does not establish guilt, shared motive or central command.')}</p><div class="actor-meta"><span>${Number(actor.records||1)} linked record${Number(actor.records||1)===1?'':'s'}</span><a href="${escapeHtml(evidenceRoute)}">Open evidence</a></div></article>`;
}).join('');
const scenarioLines=scenarios.map(s=>`<li><strong>${escapeHtml(s.title)}:</strong> ${escapeHtml(s.plausibilityBand)}</li>`).join('')||'<li>No scenario met the minimum evidence and falsifier requirements for this build.</li>';
...
const section=`<!-- homepage-command-surface:start --><section id="homepage-command-surface" class="section wrap"><div class="command-hero"><div class="eyebrow">Current Mission Conclusion · ${escapeHtml(String(wall.updated||new Date().toISOString()).slice(0,10))}</div><h2>What the evidence is pointing toward now</h2><p class="command-conclusion">${escapeHtml(evidenceConclusion)}</p><div class="confidence-strip"><strong>Analytic confidence: ${confidence}/100</strong><span>${escapeHtml(confidenceBand)}</span><small>Pattern support — not event probability.</small></div><div class="command-grid"><article class="command-panel"><span class="label">Evidence-led conclusion</span><h3>Who is involved and why it matters</h3><div class="actor-intel-grid">${actorCards}</div><p class="home-boundary">Every entry identifies a named actor, documented action, significance and evidence route. These records do not by themselves prove guilt, shared motive or central command.</p><a class="btn" href="daily-command-brief.html">Open Full Daily Brief</a></article><article class="command-panel speculation"><span class="label">Clearly labelled speculation</span><h3>Likely trajectory if the pattern continues</h3><p>${escapeHtml(speculation)}</p><ul>${scenarioLines}</ul><p class="home-boundary"><strong>Counterpoint:</strong> ${escapeHtml(counterpoint)}</p><a class="btn alt" href="data/speculative-intelligence-synthesis.json">Open Scenario Matrix</a></article></div></div><div class="command-block"><div class="command-title"><div><span class="eyebrow">Canonical scores only</span><h2>Risk clocks over 90%</h2></div><a class="btn alt" href="timers.html">Open All Timers</a></div><div class="home-clock-grid">${clockCards}</div></div><div class="command-block"><div class="command-title"><div><span class="eyebrow">Seven-day window only</span><h2>Latest news and source changes</h2></div><a class="btn alt" href="live-intel.html">Open Live Intel</a></div><div class="home-news-grid">${newsCards}</div></div><style>.command-hero,.command-block{border:1px solid rgba(216,181,106,.28);border-radius:24px;padding:1.25rem;margin-bottom:1rem;background:linear-gradient(145deg,rgba(20,4,4,.9),rgba(0,0,0,.94));box-shadow:0 20px 70px rgba(0,0,0,.28)}.command-conclusion{font-size:clamp(1.05rem,2vw,1.35rem);line-height:1.65}.confidence-strip{display:flex;gap:.6rem;align-items:center;flex-wrap:wrap;border:1px solid rgba(216,181,106,.25);border-radius:14px;padding:.75rem;margin:1rem 0}.confidence-strip small{color:#c9ba91}.command-grid{display:grid;grid-template-columns:1.15fr .85fr;gap:1rem}.command-panel{border:1px solid rgba(216,181,106,.18);border-radius:16px;padding:1rem;background:rgba(255,255,255,.025)}.command-panel.speculation{border-color:rgba(190,55,55,.5);background:rgba(120,0,0,.12)}.actor-intel-grid{display:grid;gap:.7rem;margin:.8rem 0}.actor-intel-card{border:1px solid rgba(216,181,106,.22);border-left:3px solid rgba(216,181,106,.72);border-radius:12px;padding:.8rem;background:rgba(0,0,0,.28)}.actor-intel-card h4{margin:0 0 .45rem;color:#f0d28b;font-size:1.04rem}.actor-intel-card p{margin:.3rem 0;line-height:1.45}.actor-action{font-size:.95rem}.actor-meta{display:flex;justify-content:space-between;gap:.8rem;align-items:center;margin-top:.55rem;font-size:.78rem;color:#c9ba91}.actor-meta a{color:#f0d28b}.command-title{display:flex;justify-content:space-between;gap:1rem;align-items:end;flex-wrap:wrap}.home-clock-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:.75rem}.home-clock{border:1px solid rgba(216,181,106,.3);border-radius:16px;background:rgba(20,4,4,.75)}.home-clock a{display:flex;align-items:center;justify-content:space-between;gap:1rem;padding:1rem;text-decoration:none}.home-clock strong{font-size:1.75rem;color:#f0d28b}.home-news-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(285px,1fr));gap:.85rem}.home-news-card{border:1px solid rgba(255,255,255,.12);border-radius:16px;padding:1rem;background:rgba(255,255,255,.025)}.home-news-meta{display:flex;gap:.5rem;justify-content:space-between;font-size:.8rem;color:#c9ba91}.home-boundary{font-size:.86rem;color:#bfb18c}@media(max-width:900px){.command-grid{grid-template-columns:1fr}}@media(max-width:760px){.actor-meta{align-items:flex-start;flex-direction:column}}</style></section><!-- homepage-command-surface:end -->`;
...
const output={ok:true,updated:new Date().toISOString(),purpose:'Keep the homepage focused on the latest evidence-led conclusion, named actors and institutions, exact documented roles, laws or filings, bounded trajectory speculation, canonical clocks over 90%, and current news within seven days.',evidenceConclusion,speculation,counterpoint,analyticConfidence:{score:confidence,band:confidenceBand,meaning:'Pattern support, not event probability.'},actors:actors.map(a=>({name:a.name,documentedRole:a.documentedRole,action:a.action,authority:a.authority,instrument:a.instrument,whyItMatters:a.whyItMatters,boundary:a.boundary,sourceRoute:a.sourceRoute})),scenarios:scenarios.map(s=>({id:s.id,title:s.title,status:s.status,plausibilityBand:s.plausibilityBand})),criticalClocks:critical.map(clock=>({slug:clock.slug,title:clock.title,score:clock.score})),latestNews,rules:{genericActionLabelsForbidden:true,placeholderActorsForbidden:true,namedActorOrInstitutionRequired:true,actorContextRequired:['documented action or role','why it matters','evidence boundary','evidence route'],clockThreshold:'strictly greater than 90',clockSource:'data/clock-wall.json canonical scores',newsFreshnessDays:7,staleNewsAllowed:false,speculationMustBeLabelled:true,confidenceIsProbability:false}};
fs.writeFileSync(file('data/homepage-command-surface.json'),JSON.stringify(output,null,2));
if (genericActor.test(homepage) || placeholderActor.test(homepage)) throw new Error('Generic actor placeholder leaked into homepage');
console.log(`Homepage command surface built: ${critical.length} clocks over 90%, ${latestNews.length} current news items, ${actors.length} named actor records and ${scenarios.length} bounded scenarios.`);
