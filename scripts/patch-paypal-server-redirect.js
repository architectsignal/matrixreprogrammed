...
const canonicalClient = `(() => {
  ...
  function renderBillingAction(config){for(const tier of paidTiers){const container=$(\`paypal-button-\${tier}\`);if(!container)continue;container.replaceChildren();const billingLink=document.createElement('a');billingLink.className='btn';billingLink.href=config.billingUrl||'billing-dashboard.html';billingLink.textContent='Manage billing';container.append(billingLink)}status('Paid access is already active. Use the billing dashboard to manage or cancel it.','good')}
  ...
  async function start(){showReturnMessage();try{status('Checking PayPal checkout readiness…');const config=await api('/api/paypal/config');if(config.paidAccess&&config.currentSubscription){renderBillingAction(config);return}if(!config.configured){status('PayPal credentials and webhook are not configured yet. No payment can be taken.','warning');return}if(!config.checkoutEnabled){status(\`PayPal \${config.environment} checkout is installed but still disabled behind the activation switches. No payment can be taken.\`,'warning');return}renderCheckoutActions();status(\`PayPal \${config.environment} checkout is enabled. Choose a tier to continue directly to PayPal’s secure approval page.\`,config.environment==='live'?'good':'warning')}catch(error){if(error.authentication){status('Log in first. The paid membership buttons will appear immediately after your verified Matrix session is active.','warning');document.querySelectorAll('[data-paypal-login]').forEach(node=>node.hidden=false);renderLoginActions()}else status(error.message||'PayPal checkout is unavailable.','danger')}}
  start();
})();
`;
