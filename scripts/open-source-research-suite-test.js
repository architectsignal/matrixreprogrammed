...
const timelineFallbackMessage = /(?:accessible|searchable) event list (?:remains fully usable|is ready now)/i.test(timelineJs);
const timelineListRuntime = /function\s+renderList\s*\(/.test(timelineJs) && /elements\.list\.innerHTML/.test(timelineJs);
const timelineListSurface = timelinePage.includes('Accessible event list') && timelinePage.includes('id="timeline-list"');
check('Timeline has accessible list fallback', timelineFallbackMessage && timelineListRuntime && timelineListSurface, `message=${timelineFallbackMessage}; runtime=${timelineListRuntime}; surface=${timelineListSurface}`);
