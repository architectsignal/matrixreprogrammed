...
const requiredSource = [
  ...
  'src/worker.js','src/worker-forum-persistence.js','src/worker-member-experience.js','src/worker-paypal-subscriptions.js','src/worker-production.js','src/worker-production-autonomy.js','src/worker-ai-management.js','src/worker-release-metadata.js','src/worker-public-investigation.js','src/public-investigation-contract.js','src/worker-living-matrix.js','src/worker-value-hunter.js','src/worker-permissionless-value.js','src/matrix-event-emitter.js','ai-management/living-matrix/living-matrix-cycle.mjs','ai-management/value-hunter/value-hunter-core.mjs','ai-management/value-hunter/financial-firewall.mjs','ai-management/value-hunter/value-collector.mjs','ai-management/value-hunter/permissionless/permissionless-core.mjs','ai-management/value-hunter/permissionless/permissionless-value-director.mjs','ai-management/value-hunter/permissionless/adapters/morpho-liquidation-v1.mjs','ai-management/provider-adapters/value/official-html-links.mjs',
  ...
  'migrations/public_investigation_api.sql','migrations/phase14_living_matrix.sql','migrations/phase15_matrix_value_hunter.sql','migrations/phase16_permissionless_value_harvester.sql',
...
for (const text of ["import productionWorker from './worker-production.js';","import aiManagementWorker from './worker-ai-management.js';","import { handleLivingMatrixRoute, isLivingMatrixAdminRoute, isLivingMatrixPublicRoute, runScheduledLivingMatrix } from './worker-living-matrix.js';","import { handleValueHunterRoute, isValueHunterRoute, runScheduledValueHunter } from './worker-value-hunter.js';","import { handlePermissionlessHarvesterRoute, isPermissionlessHarvesterRoute, runScheduledPermissionlessHarvester } from './worker-permissionless-value.js';",'return productionWorker.fetch(request, env, ctx);','productionWorker.scheduled','aiManagementWorker.scheduled','runScheduledLivingMatrix','runScheduledValueHunter','runScheduledPermissionlessHarvester','await Promise.all([productionTask, autonomyTask]);']) {
...
for (const text of ['binding = "FORUM_POSTS"','binding = "MEMBERS_DB"','directory = "./_site"','keep_vars = true','AI_RESOURCE_ZERO_SPEND_LOCK = "true"','MATRIX_PERMISSIONLESS_VALUE_ENABLED = "false"','MATRIX_PERMISSIONLESS_AUTO_EXECUTION_ENABLED = "false"']) {
