...
const requireText=(file,marker,label)=>{const value=read(file);if(!value.includes(marker))hard.push(`${label} missing marker: ${marker}`)};
...
const requiredSource=['behind-the-curtain-access.html','behind-the-curtain-access-v2.js','data/behind-the-curtain-pyramid.json','data/behind-the-curtain-people-registry.json'];
const requiredBuilt=['behind-the-curtain-access.html','behind-the-curtain-access','behind-the-curtain-access-v2.js','data/behind-the-curtain-pyramid.json','data/behind-the-curtain-people-registry.json'];
requiredSource.forEach(rel=>need(source(rel),'source asset'));
requiredBuilt.forEach(rel=>need(built(rel),'built asset'));
requireText(source('behind-the-curtain-access.html'),'behind-the-curtain-access-v2.js','source Pyramid HTML');
requireText(source('behind-the-curtain-access.html'),'SELECT A LEVEL. NAME ITS OPERATORS.','source Pyramid HTML');
requireText(built('behind-the-curtain-access.html'),'behind-the-curtain-access-v2.js','built Pyramid HTML');
requireText(built('behind-the-curtain-access.html'),'SELECT A LEVEL. NAME ITS OPERATORS.','built Pyramid HTML');
requireText(source('behind-the-curtain-access-v2.js'),'renderSelectedTier','source Pyramid renderer');
requireText(built('behind-the-curtain-access-v2.js'),'renderSelectedTier','built Pyramid renderer');
...
validatePeople(people,'source people registry');
validatePeople(builtPeople,'built people registry');
...
const report={ok:hard.length===0,checkedAt:new Date().toISOString(),hardIssues:hard,peopleCount:people?.people?.length||0,levels:pyramid?.levels?.length||0,chokePoints:pyramid?.chokePoints?.length||0};
...
console.log(`Behind the Curtain production guard PASS: ${report.peopleCount} people, ${report.levels} levels, ${report.chokePoints} choke points, independent renderer and built assets verified.`);
