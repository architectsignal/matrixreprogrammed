...
fs.writeFileSync(path.join(root, peopleMdOut), [
  ...
  '',
  '## Boundary',
  peopleIndex.boundary || data.boundary || '',
  '',
  '## Evidence Class Legend',
  ...(peopleIndex.evidenceClasses || []).map(item => `- ${item}`),
  '',
  '## People / Entities',
  ...(peopleIndex.people || []).map(item => `- ${item.name} — ${item.type}. What the record shows: ${item.recordShows} Boundary: ${item.boundary}`)
].join('\n'));
fs.writeFileSync(path.join(root, cockpitMdOut), [
  ...
  '',
  '## Boundary',
  fileCockpit.boundary || data.boundary || '',
  '',
  '## Open These File Doors First',
  ...(fileCockpit.doors || []).map(door => `- ${door.title}: ${door.url} — ${door.use}`),
  '',
  '## How To Use The Cockpit',
  ...(fileCockpit.howToUse || []).map(rule => `- ${rule}`)
].join('\n'));
...
console.log(`Enhanced Epstein Evidence Watch with ${(data.watchSources || []).length} source lanes, ${(data.bulletins || []).length} bulletins, ${(emailSignals.signals || []).length} email/network signals, ${(peopleIndex.people || []).length} people/entity cards, ${(fileCockpit.doors || []).length} actual file doors, Evidence Boundary marker, and download outputs.`);