...
const priorSuccessAt = new Date(Date.now() - 25 * 60 * 60 * 1000).toISOString();
...
writeJson(path.join(fixture, 'data/source-snapshot-index.json'), { updated: observedAt, sources: { test: { sourceId: 'test', label: 'Test Source', url: 'https://example.test/source?api_key=SECRET123', lane: 'oversight-audit', authority: 'primary-official', lastAttempt: observedAt, lastSuccess: priorSuccessAt, availability: 'unavailable', consecutiveFailures: 2, statusCode: 503, lastErrorCategory: 'retrieval-failure' } } });
writeJson(path.join(fixture, 'data/source-change-ledger.json'), { updated: observedAt, changes: [{ id: 'abc', sourceId: 'test', sourceLabel: 'Test Source', sourceUrl: 'https://example.test/source?api_key=SECRET123', finalUrl: 'https://example.test/source?token=SECRET456', lane: 'oversight-audit', authority: 'primary-official', detectedAt: observedAt, changeType: 'records-removed', previousHash: 'a'.repeat(64), currentHash: 'b'.repeat(64), removals: ['Old record'], additions: [], removalCount: 1, additionCount: 0, established: 'One passage disappeared.', notEstablished: 'This is not proof of wrongdoing.', evidenceGrade: 'B', status: 'missing-or-altered-record' }] });
writeJson(path.join(fixture, 'data/source-change-public.json'), { updated: observedAt, summary: {}, changes: [] });
writeJson(path.join(fixture, 'data/source-snapshots/test/hash.json'), { sourceId: 'test', sourceUrl: 'https://example.test/source?api_key=SECRET123', finalUrl: 'https://example.test/source?token=SECRET456', retrievedAt: observedAt, rawHash: 'a'.repeat(64), normalizedHash: 'b'.repeat(64), normalizedText: 'Public record' });
