...
function wildcardMatch(pattern, value) {
  const safe = String(pattern).replace(/[.+^${}()|[\]\\]/g, '\\$&').replace(/\*/g, '.*');
...
function ruleMatches(rule, file, figure) {
  ...
  const figureOk = !(rule.figurePatterns || []).length || (rule.figurePatterns || []).some(pattern => context.includes(String(pattern).toLowerCase()));
  return fileOk && figureOk;
}
function classifyFigure(file, figure) {
  const matched = rules.filter(rule => ruleMatches(rule, file, figure));
  if (!matched.length) return { ...figure, classification: 'missing-source-rule', updatePolicy: rulesData.defaultAction || 'flag-for-review', matchedRules: [] };
  const strongest = matched.find(r => r.updatePolicy === 'auto-update-allowed') || matched.find(r => r.updatePolicy === 'manual-review-only') || matched[0];
  return { ...figure, classification: strongest.updatePolicy || 'manual-review-only', updatePolicy: strongest.updatePolicy || 'manual-review-only', matchedRules: matched.map(r => ({ id: r.id, label: r.label, updatePolicy: r.updatePolicy, sourceType: r.sourceType, reviewNote: r.reviewNote })) };
}
const pages = htmlFiles.map(file => {
  ...
  return {
    ...
    manualReviewCount,
    staticCount,
    percentages: percentages.map(fig => classifyFigure(file, fig)),
    money: money.map(fig => classifyFigure(file, fig)),
    crisisFigures: crisisFigures.map(fig => classifyFigure(file, fig)),
    dates: dates.slice(0, 6),
    needsSourceRule: missingRuleCount > 0,
    recommendation
  };
}).filter(p => p.figureCount || p.priority !== 'Low')
  .sort((a, b) => ({ High: 0, Medium: 1, Low: 2 }[a.priority] - { High: 0, Medium: 1, Low: 2 }[b.priority]) || b.missingRuleCount - a.missingRuleCount || b.figureCount - a.figureCount);
...
const report = {
  ...
  pagesWithMissingRules: pages.filter(p => p.missingRuleCount > 0).length,
  autoUpdateEligibleFigures: pages.reduce((sum, p) => sum + p.autoAllowedCount, 0),
  manualReviewFigures: pages.reduce((sum, p) => sum + p.manualReviewCount, 0),
  missingRuleFigures: pages.reduce((sum, p) => sum + p.missingRuleCount, 0),
  pages
};
function stableSignature(obj) {
  return JSON.stringify({
    ...
    pages: obj.pages.map(p => ({ file: p.file, priority: p.priority, figureCount: p.figureCount, missingRuleCount: p.missingRuleCount, autoAllowedCount: p.autoAllowedCount, manualReviewCount: p.manualReviewCount, figures: [...p.percentages, ...p.money, ...p.crisisFigures].map(x => `${x.value}:${x.classification}:${(x.matchedRules || []).map(r => r.id).join('|')}`) }))
  });
}
...
if (previous && stableSignature(previous) === stableSignature(report)) {
  console.log(`Site freshness scan checked ${htmlFiles.length} pages: no meaningful figure-rule changes.`);
  process.exit(0);
}
...
const md = [
  ...
  '',
  '## High Priority Pages',
  ...pages.filter(p => p.priority === 'High').slice(0, 30).map(p => `- ${p.file}: ${p.figureCount} figure/stat markers; ${p.missingRuleCount} missing rules. ${p.recommendation}`),
  '',
  '## Missing Source Rules',
  ...pages.filter(p => p.missingRuleCount > 0).slice(0, 40).map(p => `- ${p.file}: ${p.missingRuleCount} figure(s) need source rules.`),
  '',
...
const cards = pages.slice(0, 100).map(p => {
  const examples = [...p.percentages, ...p.money, ...p.crisisFigures].slice(0, 8).map(x => `${x.value} — ${x.classification} — ${x.context}`).join('\n');
  return `<article class="card ${p.priority === 'High' ? 'redline' : ''}"><span class="label">${esc(p.priority)} · ${p.figureCount} figures · ${p.missingRuleCount} missing rules</span><h3>${esc(p.file)}</h3><p>${esc(p.recommendation)}</p><details><summary>Detected examples</summary><p>${esc(examples)}</p></details></article>`;
}).join('');
