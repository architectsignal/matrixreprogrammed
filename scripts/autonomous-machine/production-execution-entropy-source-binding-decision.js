...
const sourceBindingDecisionStore = new ProductionExecutionEntropySourceBindingDecisionStore(
  path.join(runtimeDir, 'production-execution-entropy-source-binding-decisions.jsonl'),
);
...
function usage() {
  process.stdout.write([
    'Phase 1.23 signed human decisions for entropy-source-binding requests',
    '',
    'Commands:',
    '  list',
    '  show <decision-id-or-request-id>',
    '  decide <source-binding-request-id> <approve|reject> --reviewer <name> --role <role> --note <reason>',
    '    [--all-reviews-complete]',
    '    [--request-window-reviewed] [--source-class-binding-reviewed]',
    '    [--no-provider-reviewed] [--no-implementation-reviewed]',
    '    [--no-api-device-syscall-reviewed] [--no-network-source-reviewed]',
...
function verifiedDecisions(signingKey) {
  const verification = sourceBindingDecisionStore.verify(signingKey);
  if (!verification.valid) throw new Error(`Entropy source binding decision ledger verification failed: ${verification.reason}`);
  return sourceBindingDecisionStore.readRecords();
}
function run() {
  const [command = 'help', ...args] = process.argv.slice(2);
  if (['help', '--help', '-h'].includes(command)) { usage(); return; }
  const requestSigningKey = requiredEnvironment('AIM_EXECUTION_ENTROPY_SOURCE_BINDING_REQUEST_SIGNING_KEY');
  const decisionSigningKey = requiredEnvironment('AIM_EXECUTION_ENTROPY_SOURCE_BINDING_DECISION_SIGNING_KEY');
  if (command === 'verify') {
    process.stdout.write(`${JSON.stringify(sourceBindingDecisionStore.verify(decisionSigningKey), null, 2)}\n`);
    return;
  }
  if (command === 'list') {
    const records = verifiedDecisions(decisionSigningKey).map((record) => ({
      id: record.id,
      sourceBindingRequestId: record.payload.sourceBindingRequest.id,
  ...
  if (command === 'show') {
    const id = args[0];
    if (!id) throw new Error('show requires a decision id or source-binding request id');
    const record = verifiedDecisions(decisionSigningKey).find(
      (item) => item.id === id || item.payload.sourceBindingRequest.id === id,
    );
    if (!record) throw new Error(`Entropy source binding decision not found: ${id}`);
  ...
  if (command === 'decide') {
    ...
    if (!requestId || !decision) throw new Error('decide requires source-binding request id and approve or reject');
    const allReviews = flag(args, '--all-reviews-complete');
    const result = decideProductionExecutionEntropySourceBinding({
      entropySourceBindingRequestId: requestId,
      entropySourceBindingRequestStore: sourceBindingRequestStore,
      entropySourceBindingDecisionStore: sourceBindingDecisionStore,
      auditLog,
      repositoryRoot: rootDir,
      upstreamIntegrityChecks: upstreamIntegrityChecks(),
      entropySourceBindingRequestSigningKey: requestSigningKey,
      entropySourceBindingDecisionSigningKey: decisionSigningKey,
      entropySourceBindingDecisionSigningKeyId:
        process.env.AIM_EXECUTION_ENTROPY_SOURCE_BINDING_DECISION_SIGNING_KEY_ID
      ...
      completedReviews: {
        sourceBindingRequestWindowReview: allReviews || flag(args, '--request-window-reviewed'),
        approvedSourceClassBindingReview: allReviews || flag(args, '--source-class-binding-reviewed'),
        ...
        noApiDeviceSyscallSelectionReview: allReviews || flag(args, '--no-api-device-syscall-reviewed'),
        noNetworkSourceReview: allReviews || flag(args, '--no-network-source-reviewed'),
