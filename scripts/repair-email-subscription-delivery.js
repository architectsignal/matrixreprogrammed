...
function repairEmailWorker(source) {
  ...
  const signupStart = "const email=clean(input.email,254).toLowerCase();if(!validEmail(email))return json({ok:false,error:'Valid email required'},400);if(!bool(input.consent,false))return json({ok:false,error:'Explicit email consent is required'},400);let member=await upsertSignupMember(env,{...input,email},{resubscribe});";
  const signupStartSafe = "const email=clean(input.email,254).toLowerCase();if(!validEmail(email))return json({ok:false,error:'Valid email required'},400);if(!bool(input.consent,false))return json({ok:false,error:'Explicit email consent is required'},400);const existingBeforeSignup=await memberByEmail(env,email);let member=await upsertSignupMember(env,{...input,email},{resubscribe});";
  source = replaceOnce(source, signupStart, signupStartSafe, 'Existing subscriber snapshot');
