...
const FETCH_TIMEOUT_MS = Number(process.env.MATRIX_FEED_TIMEOUT_MS || 8000);
const MAX_EVENTS_PER_PULL = Number(process.env.MATRIX_FEED_MAX_EVENTS || 8);
...
async function main() {
  ...
  for (const lane of lanes) {
    ...
    const record = { updated, lane: lane.id, name: lane.name, ok: pull.ok, status: pull.status, url: lane.url, source_type: lane.recordType, evidence_grade: lane.evidenceGrade, itemCount: items.length, raw: pull.ok ? pull.body : null, error: pull.error || null };
  ...
  const recordEvents = { updated, title: 'Public Record Events', purpose: 'Normalized events produced by the public-record feed runner. These are additive machine inputs and do not replace Live Intel, Daily Brain or existing update systems.', boundary: 'Events must keep evidence grades. News signals are not findings.', pullSummary: pulls, events: events.slice(0, 120) };
  const entityObservations = { updated, title: 'Entity Observations', purpose: 'Repeated names detected from public-record pulls. These are candidates for Power Entity Engine review, not proof of wrongdoing.', observations: [...observationsByName.values()].sort((a, b) => b.count - a.count).slice(0, 120) };
  ...
  write('data/source-pulls/source-pull-index.json', JSON.stringify({ updated, pulls }, null, 2));
...
main().catch(error => {
  ...
  const fallback = { updated, title: 'Public Record Events', purpose: 'Fallback output. Feed runner failed safely and did not replace existing updates.', boundary: 'No events should be treated as evidence from this run.', pullSummary: [], events: [] };
  write('data/record-events.json', JSON.stringify(fallback, null, 2));
  write('data/entity-observations.json', JSON.stringify({ updated, title: 'Entity Observations', observations: [] }, null, 2));
  write('data/source-pulls/source-pull-index.json', JSON.stringify({ updated, pulls: [] }, null, 2));
