...
if (!source.includes('function commercialLegalReady')) {
  ...
  report.patched.push('commercial-legal-helper');
}
...
const activationStateReplacement = `async function activationState(env){
  ...
  return{environment:target,configured:configured(env),environmentSwitch,databaseSwitch:Boolean(setting.checkout_enabled),confirmation,commercialLegalReady:legalReady,plansReady,checkoutEnabled:configured(env)&&environmentSwitch&&Boolean(setting.checkout_enabled)&&confirmation&&legalReady&&plansReady,setting,plans:planRows};
}`;
if (!source.includes('commercialLegalReady:legalReady')) {
  ...
  report.patched.push('activation-state-legal-gate');
}
...
const checkoutReplacement = `async function checkoutIntent(request,env){
  const required=await requireAuth(request,env);if(required.response)return required.response;
  const state=await activationState(env);
  if(!state.checkoutEnabled)return json({ok:false,configured:state.configured,environment:state.environment,commercialLegalReady:state.commercialLegalReady,error:'PayPal checkout is disabled until every activation and commercial-readiness gate passes'},503);
...
if (!source.includes('Verified commercial operator information and the protected commercial legal confirmation')) {
  ...
  report.patched.push('live-activation-commercial-gate');
}
...
for (const marker of [
  ...
  'protected commercial legal confirmation'
]) need(source.includes(marker), `Patched PayPal Worker missing marker: ${marker}`);

fs.writeFileSync(workerPath, source);
fs.mkdirSync(path.dirname(reportPath), { recursive: true });
fs.writeFileSync(reportPath, JSON.stringify(report, null, 2));
console.log(`Commercial PayPal guard patched: ${report.patched.length} change group(s); durable consent and live legal gate present.`);
