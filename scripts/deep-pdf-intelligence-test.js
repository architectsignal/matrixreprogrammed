...
for(const item of index.pdfs){
  ...
  if(!located){missingManifests.push(output);continue;}
...
assert.deepEqual(missingManifests,[],`Internal report manifests missing for: ${missingManifests.slice(0,20).join(', ')}${missingManifests.length>20?' …':''}`);
