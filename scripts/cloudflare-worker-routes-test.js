...
for (const marker of ['/api/investigate','matrix_public_investigations','validatePublicInvestigationResult','MATRIX_PUBLIC_INVESTIGATION_LOCAL_ENRICHMENT_ENABLED','no-eligible-model-evidence-fallback','fallback_used']) needText('src/worker-public-investigation.js', marker, `public investigation boundary ${marker}`);
...
for (const marker of ['directory = "./_site"','binding = "ASSETS"','binding = "FORUM_POSTS"','binding = "MEMBERS_DB"','database_name = "matrix-members"','c6e465d3-4e36-4a00-b8f8-309447240c52','keep_vars = true','AI_RESOURCE_ZERO_SPEND_LOCK = "true"','MATRIX_PUBLIC_INVESTIGATION_ENABLED = "true"','MATRIX_VALUE_HUNTER_ENABLED = "true"','MATRIX_VALUE_AUTO_COLLECTION_ENABLED = "true"','MATRIX_OPERATING_SYSTEM_ENABLED = "true"']) needText('wrangler.toml', marker, `wrangler.toml marker ${marker}`);
