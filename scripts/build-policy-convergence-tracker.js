...
function nav() {
  return `<header class="wrap topbar"><a class="brand" href="index.html"><img src="sigil.png" alt="Matrix Reprogrammed sigil" /> MATRIX REPROGRAMMED</a><nav class="nav"><a href="index.html">Home</a><a href="start-here.html">Start Here</a><a href="control-system-tracker.html">Control Tracker</a><a href="timers.html">Timers</a><a href="power-atlas.html">Power Atlas</a><a href="atlas-index.html">Atlas Nodes</a><a href="evidence-vault.html">Evidence Vault</a><a href="news.html">Intel Desk</a><a href="search.html">Search</a></nav></header>`;
}
...
if (fs.existsSync(path.join(root, 'search-index.json'))) {
  ...
  if (!index.some(item => item.url === 'control-system-tracker.html')) {
    index.push({
      ...
      keywords: ['Agenda 2030', 'digital ID', 'CBDC', 'policy mandates', 'global standards', 'security powers', 'climate policy', 'health governance', 'migration systems', 'food systems', 'risk timers']
    });
    fs.writeFileSync(path.join(root, 'search-index.json'), JSON.stringify(index, null, 2));
  }
}

require('./link-policy-convergence-timers.js');
console.log(`Built policy convergence tracker with ${(data.lanes || []).length} lanes and ${(data.crossLinks || []).length} cross-links, then linked lanes into global risk timers.`);
