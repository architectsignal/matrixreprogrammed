...
const report = {
  ...
  boundary: 'This is the final mutation and audit step for the exact _site bundle and Worker configuration. No later generator may overwrite repaired public output, €1–€5,000 voluntary support boundaries, Brevo sender-domain readiness, reply-to support, readiness reporting, search assets, or the disabled marketing, transactional-email, donation and live-PayPal switches.'
};
fs.mkdirSync(path.join(root, 'downloads'), { recursive: true });
fs.writeFileSync(path.join(root, 'downloads', 'final-release-sanitize.json'), `${JSON.stringify(report, null, 2)}\n`);
console.log('Final release sanitation passed for the deployable _site bundle, voluntary support store, Brevo transactional readiness and operational audit, and Cloudflare safety configuration.');
