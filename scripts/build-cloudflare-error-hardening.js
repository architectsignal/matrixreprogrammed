...
function syntaxOk(source) {
  const tmp = path.join(root, '.worker-hardening-check.mjs');
  try {
    fs.writeFileSync(tmp, source);
    const checked = spawnSync(process.execPath, ['--check', tmp], { encoding: 'utf8' });
    if (checked.status !== 0) console.warn(`Worker hardening skipped invalid patch: ${(checked.stderr || checked.stdout || '').trim()}`);
