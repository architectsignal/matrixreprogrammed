...
for (const relative of ['reverse-accountability-search.js', 'power-supply-chain.js', 'evidence-half-life.js', 'power-diff.js']) {
  for (const base of roots) {
    const file = path.join(base, relative);
...
fs.writeFileSync(path.join(root, 'downloads', 'accountability-innovation-link-finalization.json'), JSON.stringify({
  ...
  linkedSystems: ['reverse-accountability-search', 'power-supply-chain', 'evidence-half-life', 'power-diff', 'red-team-mirror']
}, null, 2) + '\n');
