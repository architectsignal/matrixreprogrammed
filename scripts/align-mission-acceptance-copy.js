...
function patchNewsletterJs(source) {
  let next = source.replace('Saved. Check your inbox to verify your email. Once verified, today’s Daily Control Brief will be sent immediately.', 'Saved. Check your inbox to verify your email and activate reports. Once verified, today’s Daily Control Brief will be sent immediately.').replace('Saved. Check your inbox to verify your email and activate the selected briefings.', 'Saved. Check your inbox to verify your email and activate reports.');
...
function readFirst(routes) { for (const base of roots) for (const route of routes) { const file = path.join(base, route); if (fs.existsSync(file) && fs.statSync(file).isFile()) return fs.readFileSync(file, 'utf8'); } return ''; }
function runRequired(script, args = []) {
  const result = spawnSync(process.execPath, [path.join(root, script), ...args], { cwd: root, encoding: 'utf8', env: process.env, maxBuffer: 40 * 1024 * 1024 });
  if (result.stdout) process.stdout.write(result.stdout);
  if (result.stderr) process.stderr.write(result.stderr);
  if (result.status !== 0) throw new Error(`${script} ${args.join(' ')} failed`);
}
function copyToOutput(relative) { const source = path.join(root, relative); const outputRoot = path.join(root, '_site'); if (!fs.existsSync(source) || !fs.existsSync(outputRoot)) return; const destination = path.join(outputRoot, relative); fs.mkdirSync(path.dirname(destination), { recursive: true }); fs.copyFileSync(source, destination); touched.push(path.relative(root, destination).replace(/\\/g, '/')); }
...
const checks = {
  timerScoreMeaning: timers.includes('What this score means'), timerRaiseLabel: timers.includes('What would raise it'), timerLowerLabel: timers.includes('What would lower it'),
  protectedStorage: newsletterHtml.includes('protected member database'), preferenceControl: newsletterHtml.includes('manage preferences or unsubscribe'),
  verificationActivation: newsletterJs.includes('Check your inbox to verify your email and activate reports.'), immediateDailyBrief: newsletterJs.includes('today’s Daily Control Brief will be sent immediately.')
};
const ok = Object.values(checks).every(Boolean);
fs.mkdirSync(path.join(root, 'downloads'), { recursive: true });
fs.writeFileSync(path.join(root, 'downloads', 'mission-acceptance-copy-alignment.json'), JSON.stringify({ ok, generatedAt: new Date().toISOString(), roots: roots.map(value => path.relative(root, value) || '.'), touched: [...new Set(touched)], checks }, null, 2));
