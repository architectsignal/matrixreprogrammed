(function(){
  ...
  function applyLock(){
    ...
    if (lockMessage) lockMessage.textContent = ok ? 'Signal Pass unlocked on this device. Posting is open.' : 'Posting is locked until Signal Pass is unlocked on this device.';
    if (passStatus) passStatus.textContent = ok ? 'Signal Pass unlocked. You can now post.' : 'Signal Pass not unlocked on this device yet.';
  }
  ...
  async function loadFallback(){
    ...
    const posts = localPosts().concat(seed);
