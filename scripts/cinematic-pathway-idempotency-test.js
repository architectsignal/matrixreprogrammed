...
const canonicalSection = '<!-- cinematic-pathways:start --><section class="matrix-pathways extra" aria-labelledby="matrix-pathways-title"><section class="matrix-pathways-head"><span class="matrix-pathways-eyebrow">Investigate</span><h2 id="matrix-pathways-title">Canonical pathway</h2><a class="matrix-pathways-map-link" href="investigation-pathways.html">Map</a></section><section class="matrix-pathways-sequence">Discover · Verify</section><section class="matrix-pathways-boundary">Preserve this selected content.</section></section><!-- cinematic-pathways:end -->';
const secondMarker = '<!-- cinematic-pathways:start --><section class="matrix-pathways" aria-labelledby="matrix-pathways-title-old"><section class="matrix-pathways-head"><span class="matrix-pathways-eyebrow">Newest</span><h2 id="matrix-pathways-title-old">Newest canonical pathway</h2><a class="matrix-pathways-map-link" href="investigation-pathways.html">Map</a></section><section class="matrix-pathways-sequence">Discover · Verify</section><section class="matrix-pathways-boundary">Keep the newest marker block.</section></section><!-- cinematic-pathways:end -->';

// Reproduce the production Black File corruption: a legacy outer marker wraps
// the page hero and already-marked pathway blocks, while the pathway parent
// contains nested <section> descendants whose class names begin with
// matrix-pathways-. A word-boundary regex can mistake those descendants for
// the parent and consume the H1. Balanced exact-class parsing must not.
...
try {
  ...
  for (const relative of files.slice(0, 4)) {
    ...
    assert(countExactClassToken(html, 'matrix-pathways', 'section') === 1, `${relative}: expected one pathway parent section.`);
    assert(countExactClassToken(html, 'matrix-pathways-head', 'section') === 1, `${relative}: descendant head section was not preserved.`);
    ...
    assert(countExactClassToken(html, 'matrix-pathways-sequence', 'section') === 1, `${relative}: pathway sequence section was not preserved.`);
    assert(countExactClassToken(html, 'matrix-pathways-boundary', 'section') === 1, `${relative}: pathway boundary section was not preserved.`);
    ...
    assert(/<h1>THE BLACK FILE<\/h1>/i.test(html), `${relative}: nested sections or markers consumed the public H1.`);
    assert(html.includes('black-file-public-lead'), `${relative}: nested sections or markers consumed the public lead.`);
    assert(html.includes('after-pathway'), `${relative}: nested sections or markers consumed downstream page content.`);
  }

  for (const relative of files.slice(4)) {
    const html = read(relative);
    assert(countExactClassToken(html, 'matrix-pathways', 'section') === 1, `${relative}: expected one nested pathway parent section.`);
    assert(countExactClassToken(html, 'matrix-pathways-head', 'section') === 1, `${relative}: nested descendant head section was lost.`);
