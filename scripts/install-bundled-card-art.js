...
for(const name of fs.readdirSync(packDir)){
 const match=name.match(/^(people-of-interest|puppets-of-interest)(?:-[a-z0-9-]+)?\.part(\d+)\.txt$/i);
 if(!match)continue;
 const prefix=name.replace(/\.part\d+\.txt$/i,'');
...
for(const [prefix,names] of groups){
 const parts=names.sort((a,b)=>Number(a.match(/\.part(\d+)\.txt$/i)[1])-Number(b.match(/\.part(\d+)\.txt$/i)[1]));
 ...
 receipts.push({ok:true,version:pack.version||1,batchId:pack.batchId||prefix,deckId:pack.deckId,installed:seen.size,parts:parts.length,installedAt:new Date().toISOString(),source:'versioned repository card-art pack',outputDirectory:'card-art-inbox/'});
}
