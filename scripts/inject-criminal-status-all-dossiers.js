...
function walk(directory, files = []) {
  const excluded = new Set(['.git', '.github', 'node_modules', 'data', 'downloads', 'scripts', 'tools', 'tests', 'functions', 'workers', '.wrangler', '.cache']);
...
function panelFor(route, title, subject, categories) {
  ...
  const routeLink = known && subject.dossierRoute ? subject.dossierRoute : 'criminal-investigations.html';
...
for (const page of pages) {
  ...
  coverage.push({ route: page.route, title, matchedSubject: subject?.name || '', matchStatus: subject ? 'approved-registry-subject' : 'no-approved-match', approvedRecords: subject ? array(subject.records).filter(record => record.publicationStatus === 'approved').length : 0, criminalDossierRoute: subject?.dossierRoute || '' });
}
...
const report = {
  ok: missingNativeRoutes.length === 0 && coverage.length > 0,
...
if (!report.ok) throw new Error(`Criminal dossier coverage failed: ${missingNativeRoutes.length} approved dossier routes are missing.`);
