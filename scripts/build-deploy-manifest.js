...
const metadataProof = String(process.env.RELEASE_METADATA_PROOF || '').toLowerCase() === 'true';
...
if (!metadataProof && fs.existsSync(builtSiteEntry) && fs.existsSync(moneyFinalizer)) {
...
const coreCriticalFiles = [
  'index.html', 'start-here.html', 'live-intel.html', 'security-privacy.html',
...
const generatedCriticalFiles = [
  'daily-power-conclusions.html', 'daily-investigation-conclusions.html', 'weekly-investigation-report.html',
  'daily-brain-brief.html', 'outcome-briefings.html',
...
const criticalFiles = metadataProof ? coreCriticalFiles : [...coreCriticalFiles, ...generatedCriticalFiles];
const manifest = {
  ok: true,
  commitSha,
  ...
  proofMode: metadataProof ? 'metadata-only' : 'strict-production',
  ...
  generatedCriticalFiles: Object.fromEntries(generatedCriticalFiles.map(rel => [rel, hash(rel)])),
  verificationRoutes: [
...
if (missingCritical.length) throw new Error(`Deployment manifest missing critical ${metadataProof ? 'metadata proof' : 'money or production'} files: ${missingCritical.join(', ')}`);
...
console.log(`Deployment manifest built for ${manifest.commitShort} (${manifest.proofMode}).`);
