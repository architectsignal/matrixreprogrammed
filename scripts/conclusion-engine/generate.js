...
function mechanismAnalysis(record, policy) {
  ...
  return { status, candidateText, dimensions, authorityHolders, implementationRoutes, explicitMoneyAndAuthorityRoutes: explicitRoutes, evidenceBasis, limitation: record.mechanismOfPower?.limitation || '', genericMarkers };
}
...
function missionAnalysis(record, mechanism) {
  ...
  const coordinationStatus = asArray(record.missionAssessment?.convergenceVectors).some(vector => vector.coordinationStatus === 'documented') ? 'documented' : 'not_shown';
...
function buildCounterAndMissing(record, mechanism) {
  ...
  const missingEvidence = asArray(record.missingEvidence).map(item => ({ record: item.record || text(item), whyItMatters: item.whyItMatters || 'This record could confirm, narrow, contradict or falsify the current conclusion.', effectIfFound: item.effectIfFound || 'narrow', requestRoute: item.requestRoute ?? null }));
  if (sourceAuthority(record) !== 'primary') missingEvidence.push({ record: 'Underlying primary or official record', whyItMatters: 'A lead cannot support a factual or mission conclusion until the underlying record is reviewed.', effectIfFound: 'confirm', requestRoute: null });
  if (mechanism.status === 'unestablished') missingEvidence.push({ record: 'Authority, ownership, payment, contract, mandate, access, data-control or implementation record', whyItMatters: 'This is required to establish how power operates rather than merely naming associated entities.', effectIfFound: 'upgrade', requestRoute: null });
  ...
  return {
    counterHypothesis: {
      ...
      contradictoryEvidence: asArray(record.counterAnalysis?.contradictoryEvidence),
