...
check('Newsletter consent remains explicit', email.includes("Explicit email consent is required") && membership.includes('marketing-consent'), 'No marketing subscription may be activated without explicit consent');
...
check('Dashboard exposes membership management', dashboardHtml.includes('Manage membership') && dashboardHtml.includes('href="membership.html"'), 'A logged-in free member needs a clear upgrade and billing route');
...
check('First daily brief uses detailed intelligence builder', email.includes('const firstBrief=await sendDetailedFirstDailyBrief(request,env,member);') && email.includes('email.daily.detailed_first_brief_fallback'), 'Selected daily subscribers should get the detailed brief with a safe fallback');
