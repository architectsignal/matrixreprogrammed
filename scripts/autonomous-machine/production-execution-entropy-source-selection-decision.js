...
function usage() {
  process.stdout.write([
    'Phase 1.21 signed human decisions for entropy-source-selection requests',
    '',
    'Commands:',
    '  list',
    '  show <decision-id-or-request-id>',
    '  decide <source-selection-request-id> <approve|reject> --reviewer <name> --role <role> --note <reason>',
    '    [--all-reviews-complete]',
    '    [--request-window-reviewed] [--source-class-reviewed] [--no-provider-reviewed]',
    '    [--no-network-source-reviewed] [--final-preflight-reviewed] [--scope-reviewed]',
    ...
    'It cannot select a source or provider, request entropy bytes, produce entropy, edit files, commit, deploy or publish.',
  ].join('\n') + '\n');
}
function verifiedDecisions(signingKey) {
  const verification = sourceSelectionDecisionStore.verify(signingKey);
  if (!verification.valid) throw new Error(`Entropy source selection decision ledger verification failed: ${verification.reason}`);
  return sourceSelectionDecisionStore.readRecords();
}
function run() {
  ...
  const decisionSigningKey = requiredEnvironment('AIM_EXECUTION_ENTROPY_SOURCE_SELECTION_DECISION_SIGNING_KEY');
  if (command === 'verify') {
    process.stdout.write(`${JSON.stringify(sourceSelectionDecisionStore.verify(decisionSigningKey), null, 2)}\n`);
    return;
  }
  if (command === 'list') {
    const records = verifiedDecisions(decisionSigningKey).map((record) => ({
      id: record.id,
      sourceSelectionRequestId: record.payload.sourceSelectionRequest.id,
      ...
      providerSelected: record.payload.selectionState.providerSelected,
  ...
  if (command === 'show') {
    const id = args[0];
    if (!id) throw new Error('show requires a decision id or source-selection request id');
    const record = verifiedDecisions(decisionSigningKey).find(
      (item) => item.id === id || item.payload.sourceSelectionRequest.id === id,
    );
    if (!record) throw new Error(`Entropy source selection decision not found: ${id}`);
  ...
  if (command === 'decide') {
    ...
    if (!requestId || !decision) throw new Error('decide requires source-selection request id and approve or reject');
    const allReviews = flag(args, '--all-reviews-complete');
    const result = decideProductionExecutionEntropySourceSelection({
      entropySourceSelectionRequestId: requestId,
      ...
      entropySourceSelectionDecisionSigningKey: decisionSigningKey,
      entropySourceSelectionDecisionSigningKeyId:
        process.env.AIM_EXECUTION_ENTROPY_SOURCE_SELECTION_DECISION_SIGNING_KEY_ID
        || 'production-execution-entropy-source-selection-decision-key',
      decision,
      reviewerName: option(args, '--reviewer'),
      reviewerRole: option(args, '--role'),
      reviewerNote: option(args, '--note'),
      completedReviews: {
        sourceSelectionRequestWindowReview: allReviews || flag(args, '--request-window-reviewed'),
        permittedSourceClassReview: allReviews || flag(args, '--source-class-reviewed'),
