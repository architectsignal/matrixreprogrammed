...
if (!fs.existsSync(builderPath)) throw new Error('Deep PDF intelligence builder is missing');
...
if (fs.existsSync(publicManifestDir)) throw new Error('Public PDF report-manifest directory still exists after relocation');
if (!fs.readFileSync(builderPath, 'utf8').includes(internalBuilderLine)) throw new Error('Deep PDF builder is not using the internal report-manifest directory');
