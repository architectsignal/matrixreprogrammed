...
if (!problems.length) {
  ...
  need(reconciliation.includes("require('./finalize-black-file-public-hero.js')"),
    'postbuild reconciliation does not invoke the Black File hero owner');
  ...
  need(reconciliation.indexOf("require('./finalize-black-file-public-hero.js')")
  ...
  need(!/fetch\s*\(|wrangler\s+(?:deploy|d1)|method\s*:\s*['"](?:POST|PUT|PATCH|DELETE)/i.test(finalizer),
    'Black File finalizer contains a network, deploy or application-data mutation path');
}
...
const report = {
  ...
  owners: {
    ...
    canonicalHero: 'scripts/finalize-black-file-public-hero.js',
    deployGuard: 'scripts/patch-release-metadata-routing.js',
    exhaustiveAudit: 'scripts/exhaustive-public-site-audit-v2.js'
  },
  boundary: 'The canonical Black File hero must run during ordinary npm postbuild before alias synchronization, and it must be reasserted at deploy and exhaustive-audit boundaries. The owner is local, deterministic and non-networked.',
  problems
};
