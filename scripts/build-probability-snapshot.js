...
function loadModel() {
  return readJson('data/probability-lab-core.json', { initialScenarios: [], probabilityBands: [] });
}
function loadSignals() {
  return readJson('data/probability-signal-feed-001.json', { signals: [] });
}
function compute() {
  const model = loadModel();
  const feed = loadSignals();
  const scenarios = (model.initialScenarios || []).map(s => {
    const signals = [];
    let delta = 0;
    let totalWeight = 0;
    for (const sig of feed.signals || []) {
      for (const impact of sig.affectedScenarios || []) {
        if (impact.scenarioId === s.scenarioId) {
          ...
          delta += value;
          totalWeight += weight;
          signals.push({ signalId: sig.signalId, title: sig.title, weight, impact: value, lane: sig.lane, sourceStatus: sig.sourceStatus });
        }
      }
    }
  ...
  return {
    ...
    purpose: 'Computed scenario snapshot from Probability Lab priors plus weighted signal feed.',
    ...
    inputFiles: ['data/probability-lab-core.json', 'data/probability-signal-feed-001.json'],
    scenarios,
    summary: {
...
function html(snapshot) {
  return `<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"/><meta name="viewport" content="width=device-width, initial-scale=1.0"/><title>Probability Snapshot | Matrix Reprogrammed</title><meta name="description" content="Computed probability snapshot from scenario priors and weighted signals."/><link rel="stylesheet" href="styles.css"/><link rel="stylesheet" href="fixes.css"/></head><body><canvas id="matrix"></canvas><div class="signal-face"></div><div class="veil"></div><div class="page"><header class="wrap topbar"><a class="brand" href="index.html"><img src="sigil.png" alt="Matrix Reprogrammed sigil"/> MATRIX REPROGRAMMED</a><nav class="nav"><a href="probability-lab.html">Probability Lab</a><a href="system-feed-index.html">System Feed</a><a href="record-intake-queue.html">Intake</a><a href="findings-dashboard.html">Findings</a></nav></header><main><section class="hero wrap"><div class="eyebrow">Computed Forecast</div><h1>PROBABILITY SNAPSHOT.</h1><p class="lead">Computed scenario probabilities from prior estimates and weighted signal feeds. Forecasts are provisional and change when records change.</p><div class="cta-row"><a class="btn" href="data/probability-snapshot.json">Open JSON</a><a class="btn alt" href="downloads/probability-snapshot.md">Download Markdown</a><a class="btn alt" href="probability-lab.html">Probability Lab</a></div></section><section class="section wrap split"><div class="terminal">PROBABILITY SNAPSHOT\n&gt; Updated: ${snapshot.updated}\n&gt; Scenarios: ${snapshot.summary.scenarioCount}\n&gt; Average: ${snapshot.summary.averageProbability}%\n&gt; Model: priors + weighted signals</div><aside class="card redline"><h2>Boundary</h2><p>${snapshot.boundary}</p></aside></section><section class="section wrap"><h2>Computed Scenarios</h2><div class="grid">${snapshot.scenarios.map(s => `<article class="card redline"><span class="label">${s.scenarioId} · ${s.computedProbability}% · ${s.band}</span><h3>${s.title}</h3><p><strong>Topic:</strong> ${s.topic}</p><p><strong>Base:</strong> ${s.baseProbability}%</p><p><strong>Delta:</strong> ${s.probabilityDelta > 0 ? '+' : ''}${s.probabilityDelta}</p><p><strong>Confidence:</strong> ${s.confidence}</p><p><strong>Signals:</strong> ${s.supportSignalCount}</p><p><strong>Next:</strong> ${s.nextAction}</p></article>`).join('')}</div></section></main><footer class="footer wrap"><p><strong>MATRIX REPROGRAMMED</strong> — forecast, test, update.</p></footer></div><script src="matrix.js"></script></body></html>`;
}
function md(snapshot) {
  const lines = ['# Probability Snapshot', '', `Updated: ${snapshot.updated}`, '', snapshot.boundary, '', '## Summary', '', `- Scenarios: ${snapshot.summary.scenarioCount}`, `- Average probability: ${snapshot.summary.averageProbability}%`, '', '## Scenarios', ''];
...
console.log(`Probability snapshot generated: ${snapshot.summary.scenarioCount} scenarios.`);
