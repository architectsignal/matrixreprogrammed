...
const panelPath = path.join(root, 'data', 'atlas-layer-panels.json');
...
const atlasPanels = fs.existsSync(panelPath) ? JSON.parse(fs.readFileSync(panelPath, 'utf8')) : { panels: {} };
...
function laneCard(lane = {}) {
  return `<article class="card redline"><span class="label">Current Source Lane</span><h3>${esc(lane.label || 'Lane pending')}</h3><p><strong>Examples:</strong> ${esc(lane.examples || 'Examples pending.')}</p><p><strong>Source lane:</strong> ${esc(lane.source || 'Source pending.')}</p><p><strong>Update rule:</strong> ${esc(lane.update || 'Weekly check.')}</p></article>`;
}
function panelBlock(layer) {
  const panel = atlasPanels.panels && atlasPanels.panels[layer.slug];
  if (!panel) return '';
  const categories = (panel.categories || []).map(item => `<span class="pill">${esc(item)}</span>`).join('');
  const lanes = (panel.lanes || []).map(laneCard).join('');
  return `<div class="card redline"><span class="label">Expanded Layer Brief</span><h3>${esc(panel.title || layer.title)}</h3><p>${esc(panel.summary || '')}</p><p>${categories}</p></div><div class="grid">${lanes}</div>`;
}
function layerSection(layer) {
  ...
  return `<section id="layer-${esc(layer.slug)}" class="section wrap atlas-layer-section"><div class="split"><div><span class="label">Atlas Layer</span><h2>${esc(layer.title)}</h2><p class="lead">${esc(layer.definition)}</p></div><aside class="card redline"><h3>Update Rule</h3><p>${esc(layer.updateRule || 'Review weekly and replace stale entries.')}</p></aside></div>${panelBlock(layer)}<h3 class="section-kicker">Current tracked entries</h3><div class="grid">${entries}</div></section>`;
}
...
const body = `<main><section class="hero wrap"><div class="eyebrow">Power Atlas</div><h1>ATLAS LAYERS.</h1><p class="lead">${esc(atlasLayers.pageSubtitle || 'A living public-record map for entity pages, source checks, and reader navigation.')}</p><div class="cta-row"><a class="btn" href="power-atlas.html">Power Atlas</a><a class="btn alt" href="atlas-index.html">Atlas Nodes</a><a class="btn alt" href="evidence-vault.html">Evidence Vault</a><a class="btn alt machine-data-link" href="data/atlas-layers.json">Machine-readable data</a><a class="btn alt machine-data-link" href="data/atlas-layer-panels.json">Layer panel data</a></div></section><section class="section wrap split"><div class="terminal">ATLAS LAYER STATUS\n&gt; Layers: ${(atlasLayers.layers || []).length}\n&gt; Update cadence: ${esc(atlasLayers.updateCadence || 'weekly')}\n&gt; Date checked: ${esc(atlasLayers.updated || 'pending')}\n&gt; Entity-page rule: active\n&gt; Relationship labels required\n&gt; Expanded source-lane panels: active</div><aside class="card redline"><h2>Entity Page Rule</h2><p>${esc(atlasLayers.entityPageRule || 'Every future entity page must attach to one or more Atlas Layers.')}</p><p>${esc(atlasLayers.boundary || 'Atlas entries are navigation leads, not accusations.')}</p></aside></section><section class="section wrap"><h2>Layer Navigation</h2><div class="cta-row small">${layerNav}</div></section>${(atlasLayers.layers || []).map(layerSection).join('')}</main>`;
...
patchSitemap();
patchLlms();
console.log(`Built Atlas Layers page with ${(atlasLayers.layers || []).length} layers and expanded panels.`);
