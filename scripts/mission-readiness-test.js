...
const requiredFiles = [
  ...
  'scripts/build-mission-timers.js','scripts/build-daily-brain-brief.js','scripts/patch-osint-tool-tiers.js','scripts/patch-membership-tiers.js','scripts/patch-commercial-paypal-guard.js','scripts/patch-commercial-launch-readiness.js','scripts/patch-deep-email-automation.js','scripts/patch-persistent-signal-board.js',
...
check('verification queues immediate first brief', emailLifecycle.includes('queueImmediateDailyBrief') && emailLifecycle.includes("messageKind:'first_daily_brief'") && emailLifecycle.includes('public_daily_brief!==1'));
check('every campaign has personalised controls', includesAll(emailLifecycle, ['issueReusableEmailToken','Manage preferences:','Unsubscribe:','subscriber-dashboard.html?token=']));
...
check('Signal Board requires verified Free Member session', includesAll(forumWorker, ["import { memberSessionContext } from './worker-member-experience.js';",'A verified free member account is required to post.','postingAccess:\'verified-free-member-session\'']) && includesAll(forumClient, ['/api/member/me','emailVerifiedAt','Verified Free Member account is required']));
...
check('Signal Board no longer uses device-only paid unlock', !forumClient.includes('localStorage') && !forumClient.includes('matrix_signal_pass_unlocked') && !['forum.html','dark-speculation-forum.html','epstein-alive-board.html'].some(file => read(file).includes('paypal.me')));
...
check('production receipt certifies mission systems', includesAll(receipt, ['schemaVersion: 4','Cloudflare D1 Time Travel bookmark','protectedAssetTiersWired','osintToolTiersWired','timersExplained','current-membership-tier-required']));
