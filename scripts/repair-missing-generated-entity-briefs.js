...
function runRequired(script) {
  const result = spawnSync(process.execPath, [path.join(root, script)], { cwd: root, encoding: 'utf8', env: process.env, maxBuffer: 1024 * 1024 * 40 });
...
if (!failures.length) {
  ...
  if (!failures.length) runRequired('scripts/sync-criminal-conduct-extensionless.js');
  if (!failures.length) runRequired('scripts/criminal-conduct-engine-pressure-test.js');
}

const report = { ok: failures.length === 0, generatedAt: new Date().toISOString(), referencedEntityBriefs: required.size, created, marked: [...new Set(marked)], criminalConductEngineApplied: failures.length === 0, failures, boundary: 'Missing generated entity routes receive evidence-gap briefs only. These pages do not assert wrongdoing or upgrade evidence. Every generated entity brief receives the evidence-classified Criminal Conduct & Allegations dropdown.' };
...
console.log(`Generated entity route repair passed: ${required.size} referenced routes, ${created.length} missing files created, ${marked.length} entity dossier marker(s) applied; Criminal Conduct & Allegations engine verified.`);
