...
const allowedExt = new Set(['.html','.css','.js','.json','.xml','.txt','.md','.pdf','.png','.jpg','.jpeg','.webp','.svg','.ico','.gif','.mp4','.webm','.woff','.woff2','.csv','.wasm','.pf_fragment','.pf_index','.pf_meta','.pf_filter']);
...
console.log(`Cloudflare output ready: ${count.length} deployable files, including Phase 6 market activity, PDF.js evidence reading, vis-timeline chronology, Pagefind fallback wiring, Search V3 and evidence-led network maps while private diagnostics remain excluded.`);
