...
function compactLens(title, html) {
  ...
  return `${markerStart}<section class="section wrap mission-brief-lens compact"><div class="eyebrow">Mission Lens · evidence → inference → scenario</div><h2>How ${esc(title)} fits into the wider control map</h2><p><strong>Updated:</strong> ${esc(generatedDate)}</p><div class="compact-grid"><article><h3>Documented actor map</h3>${actorList(pageActors)}<p class="mission-boundary">Only cited roles and relationships are established.</p></article><article><h3>Mechanism: relevant control pathways</h3><ul>${pagePathways.map(item => `<li><strong>${esc(item.title)}:</strong> ${esc(item.meaning)}</li>`).join('')}</ul></article><article class="mission-speculation"><h3>Why it matters: clearly labelled speculation</h3><p>${esc(pageScenario)}</p><p><strong>Analytic confidence:</strong> ${confidence}% for the site-wide convergence pattern, not for this actor and not as event probability.</p></article><article><h3>What would confirm or weaken it</h3><p><strong>Confirm:</strong> mandate, ownership, contract, procurement, technical integration, data exchange, enforcement authority or access rule.</p><p><strong>Limitation and alternative explanation:</strong> decentralisation, optional participation, open standards, competition, effective appeal or records showing no operational link would weaken the inference.</p></article></div><details><summary>Watch next: scenario thresholds and falsifiers</summary>${scenarios.slice(0,5).map(s => `<p><strong>${esc(s.title)}:</strong> ${esc(s.plausibilityBand)} — ${esc(s.boundary)}</p>`).join('')}</details><p class="mission-boundary"><strong>Evidence boundary:</strong> This page does not prove secret intent, guilt, one-world government, one-world currency, one-world religion or coordinated control.</p><style>.mission-brief-lens.compact{border:1px solid rgba(216,181,106,.22);border-radius:18px;padding:1rem;background:rgba(25,5,5,.72)}.compact-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(270px,1fr));gap:.8rem}.compact-grid article{border:1px solid rgba(216,181,106,.15);border-radius:14px;padding:.9rem}.compact-grid .mission-speculation{border-color:rgba(190,55,55,.45);background:rgba(120,0,0,.1)}</style></section>${markerEnd}`;
}
function ensureEntityEvidenceSections(relative, html) {
  if (!relative.startsWith('entity-briefs/')) return html;
  const missingRecordsPresent = /<h2[^>]*>\s*Missing Records\s*<\/h2>/i.test(html);
  const watchNextPresent = /<h2[^>]*>\s*Watch Next\s*<\/h2>/i.test(html);
  if (missingRecordsPresent && watchNextPresent) return html;
  const cards = [];
  if (!missingRecordsPresent) cards.push('<article class="card redline"><h2>Missing Records</h2><ul><li>Open and verify the primary page, complete filing, PDF, docket, contract, award notice, ownership record or registry entry before upgrading the assessment.</li><li>Record provenance, publication date, completeness, redactions and any contradictory source.</li></ul></article>');
  if (!watchNextPresent) cards.push('<article class="card"><h2>Watch Next</h2><ul><li>New official releases, court filings, corrections, contracts, ownership records and authenticated correspondence.</li><li>Evidence that confirms, weakens or replaces the current analytical inference.</li></ul></article>');
  const fallback = `<section id="entity-evidence-gap-safeguard" class="section wrap"><div class="grid">${cards.join('')}</div></section>`;
  return html.includes('</main>') ? html.replace('</main>', `${fallback}</main>`) : `${html}${fallback}`;
}
function inject(relative, full = false) {
  ...
  html = ensureEntityEvidenceSections(relative, html);
  fs.writeFileSync(target,html); return true;
}
const direct = ['daily-command-brief.html','daily-brain-brief.html','daily-brief-master.html','outcome-briefings.html','entity-daily-briefs.html','weekly-intelligence-brief.html','ai-speculative-conclusions.html'];
let patched = 0;
