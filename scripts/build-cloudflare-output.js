...
function normalizeWorkerAuditMarkers() {
  ...
  let next = before.replace('const routeAliases={', 'const routeAliases = {');
...
const priorDocumentLimit = process.env.SOURCE_DOCUMENTS_PER_RUN;
process.env.SOURCE_DOCUMENTS_PER_RUN = '0';
runRequired('Source change preservation hardening', 'scripts/harden-source-change-preservation.js');
if (priorDocumentLimit === undefined) delete process.env.SOURCE_DOCUMENTS_PER_RUN;
else process.env.SOURCE_DOCUMENTS_PER_RUN = priorDocumentLimit;
...
const requiredFiles = [
  ...
  'evidence-archive.html','evidence-archive','evidence-archive.js','evidence-citation.js','source-change-diff.js','replay/sw.js','data/evidence-archive-policy.json','data/evidence-archive-manifest.json','data/evidence-integrity-manifest.json','data/evidence-citations.json',
...
if (fs.existsSync(path.join(out, '_redirects'))) {
  console.error('Cloudflare output failed: _site/_redirects must not be deployed for Worker assets.');
  process.exit(1);
}
const privateArchivePath = path.join(out, 'evidence-archive');
...
const privatePaths = [
  ...
  'downloads/osint-worker-patch-report.json','downloads/osint-tools-test.json','downloads/research-tools-ui-patch.json',
