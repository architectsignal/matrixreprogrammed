...
for (const [route, label] of [
  ...
  if (!html.includes(`href="${route}">${label}</a>`)) throw new Error(`Homepage ${label} route is missing from the final navigation drawer.`);
  if (!startHere.includes(`href="${route}">${label}</a>`)) throw new Error(`Start Here ${label} route is missing from the final navigation drawer.`);
}
...
console.log('Primary navigation reconciled across current and legacy homepage shells; Data Lab, Book Universe, Opt-in Center, Offer Center, Download Center, research, community and safety routes preserved.');
