...
const findings = (ledger.findings || [])
  ...
  .map(item => ({
    ...item,
    _rank:
      Number(statusWeight[item.status] || 0) +
      Number(gradeWeight[item.evidenceGrade] || 0) +
      Number(item.severity || 0) * 8 +
      Math.max(0, new Date(item.published || item.lastSeen || 0).getTime() / 1e12)
  }))
...
for (const laneId of usedLanes) {
  ...
  nodes.push({
    data: {
      id: safeId('lane', laneId),
      rawId: laneId,
      type: 'lane',
      label: clean(lane.title || laneId, 90),
      description: clean(lane.description || '', 360),
...
for (const sourceId of usedSources) {
  ...
  nodes.push({
    data: {
      ...
      rawId: sourceId,
      type: 'source',
      label: clean(source.label || sourceId, 110),
      description: clean(`${source.authority || 'source'} · ${(source.frequency || []).join(' / ')}`, 220),
      lane: source.lane || '',
      route: source.url || '',
      weight: 46
    }
  ...
  if (source.lane) {
    edges.push({
      data: {
        id: safeId('edge', `${source.lane}-${sourceId}`),
        source: safeId('lane', source.lane),
        target: sourceNodeId,
        type: 'monitors',
...
for (const finding of findings) {
  ...
  nodes.push({
    data: {
      ...
      rawId: finding.id,
      type: 'finding',
      label: clean(finding.title, 120),
      description: clean(finding.conclusion || finding.summary || '', 520),
      summary: clean(finding.summary || '', 520),
      boundary: clean(finding.evidenceBoundary || '', 520),
      mechanism: clean(finding.mechanism || '', 520),
      implication: clean(finding.implication || '', 520),
      nextRecords: (finding.nextRecords || []).map(item => clean(item, 240)).slice(0, 5),
      lane: finding.lane || '',
      ...
      sourceLabel: clean(finding.sourceLabel || finding.sourceId || '', 120),
      indicators: (finding.wrongdoingIndicators || []).map(item => clean(item, 80)).slice(0, 12),
      weight: 20 + Number(finding.severity || 1) * 6
    }
  });
  if (finding.sourceId) {
    edges.push({
      data: {
        id: safeId('edge', `${finding.sourceId}-${finding.id}`),
        source: safeId('source', finding.sourceId),
  ...
  } else if (finding.lane) {
    edges.push({
      data: {
        id: safeId('edge', `${finding.lane}-${finding.id}`),
        source: safeId('lane', finding.lane),
        target: findingNodeId,
        type: 'contains',
...
const graph = {
  ...
  filters: {
    grades: [...new Set(findings.map(item => item.evidenceGrade || 'C'))].sort(),
    statuses: [...new Set(findings.map(item => item.status || 'record-update'))].sort(),
    lanes: [...usedLanes].sort()
  },
...
const csvRows = [
  ...
  ...findings.map(item => [
    ...
    item.status || 'record-update',
    Number(item.severity || 1),
    item.lane || '',
    item.sourceLabel || item.sourceId || '',
    item.published || '',
    item.itemUrl || item.sourceUrl || '',
    item.conclusion || item.summary || '',
...
fs.writeFileSync(path.join(downloadsDir, 'evidence-network-map-build.json'), JSON.stringify({
  ...
  access: 'Public evidence map. Membership tiers add convenience, history, exports and research tools rather than hiding the primary-source boundary.'
}, null, 2));
