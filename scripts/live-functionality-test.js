...
async function fetchText(route, options = {}) {
  ...
  const res = await fetch(url(route), {
    ...
    headers: { 'user-agent': 'matrix-live-functionality-test/1.0', ...(options.headers || {}) },
    ...options,
  });
...
async function submitBoardPost(board, submitRoute, feedRoute) {
  ...
  const payload = {
    board,
    name: 'Matrix Synthetic Test',
    category: 'System Test',
    title: `Synthetic live persistence test ${stamp}`,
    body: `Automated live functionality test for ${board}. This confirms Cloudflare Worker submit, KV persistence, and feed retrieval.`,
    sourceUrl: '/deploy-status.json',
    website: '',
  };
  try {
...
async function main() {
  ...
  if (Array.isArray(searchIndex)) {
    const terms = ['books', 'epstein', 'matrix'];
  ...
  if (POST_TESTS) {
    ...
    addCheck('forum persistent post tests skipped', true, { reason: 'LIVE_FUNCTIONALITY_POST_TESTS=false' });
  }
