...
function validAmount(input){const value=Number(String(input?.value||'').replace(',','.'));return Number.isFinite(value)&&value>=1&&value<=500?value:null;}
...
function bindCard(card){
  ...
  button.addEventListener('click',async()=>{
    ...
    if(!amount){setStatus(status,'Choose an amount between €1.00 and €500.00.','error');input.focus();return;}
