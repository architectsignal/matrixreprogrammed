(function(){
  ...
  function card(book){
    const direct = book.asin ? '<a class="btn" href="https://www.amazon.com/dp/' + esc(book.asin) + '" target="_blank" rel="noopener">Amazon US</a><a class="btn alt" href="https://www.amazon.co.uk/dp/' + esc(book.asin) + '" target="_blank" rel="noopener">Amazon UK</a>' : '<a class="btn" href="' + esc(amazonSearch(book.title)) + '" target="_blank" rel="noopener">Find on Amazon</a>';
  ...
  fetch('data/amazon-store-visible-books.json')
    ...
    .then(data => {
      ...
      if (count) count.textContent = books.length + ' store title' + (books.length === 1 ? '' : 's') + ' listed';
      grid.innerHTML = books.map(card).join('');
    })
    .catch(() => {
      grid.innerHTML = '<article class="card redline"><h3>Catalogue not loaded</h3><p>The Amazon Store catalogue could not be loaded.</p></article>';
    });
})();
