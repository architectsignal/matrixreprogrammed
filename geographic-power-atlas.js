...
async function init() {
  try {
    const [manifestResponse,geoResponse]=await Promise.all([fetch('data/geographic-power-atlas.json',{cache:'no-store'}),fetch('data/geographic-power-atlas.geojson',{cache:'no-store'})]);
