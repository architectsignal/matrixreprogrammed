...
function requireRegex(file, regex, label) { if (!exists(file)) return; if (!regex.test(read(file))) problems.push(`${file}: missing ${label}`); }
...
const coreData = ['data/books.json', 'data/bulletins.json', 'data/human-cost.json', 'search-index.json'];

[...corePages, ...phaseOnePages, ...coreData].forEach(requireFile);
...
for (const page of ['index.html', 'start-here.html', 'books.html', 'news.html', 'power-atlas.html', 'evidence-vault.html']) {
...
if (exists('sitemap.xml')) {
  for (const file of phaseOnePages) requireIncludes('sitemap.xml', `/${file}`, `${file} in sitemap`);
}
if (exists('llms.txt')) {
  for (const file of phaseOnePages) requireIncludes('llms.txt', `/${file}`, `${file} in llms.txt`);
}
if (exists('package.json')) {
  const pkg = json('package.json');
  const build = pkg.scripts && pkg.scripts.build || '';
  for (const step of ['build-book-system.js', 'build-homepage.js', 'build-seo-system.js', 'build-intel-desk.js', 'build-phase1-structure.js', 'cleanup-duplicates.js', 'audit-site.js', 'pressure-test-site.js']) {
    if (!build.includes(step)) problems.push(`package.json build script missing ${step}`);
  }
}
if (exists('netlify.toml')) {
  const netlify = read('netlify.toml');
  for (const step of ['build-phase1-structure.js', 'cleanup-duplicates.js', 'audit-site.js', 'pressure-test-site.js']) {
...
if (exists('data/books.json') && exists('search-index.json')) {
  ...
  if (search.length !== books.length) problems.push(`search-index.json length ${search.length} does not match live book count ${books.length}`);
...
if (problems.length) {
  console.error('\nPHASE 1 PRESSURE TEST FAILED\n');
  for (const problem of problems) console.error(`- ${problem}`);
  console.error(`\n${problems.length} issue(s) found.\n`);
  process.exit(1);
}
console.log('PHASE 1 PRESSURE TEST PASSED');
console.log('Structure pages, master navigation, sitemap, llms.txt, search index, book generation, Intel Desk, Intel Archive, Human Cost panels, timers, duplicate cleanup, and build commands passed.');
