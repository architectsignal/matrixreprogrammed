...
const maxInspectBytes=8*1024*1024;
...
function walk(dir,out=[]){
  for(const entry of fs.readdirSync(dir,{withFileTypes:true})){
    ...
    if(entry.isDirectory())walk(target,out);
      ...
      if(inspectExtensions.has(path.extname(rel).toLowerCase()))out.push(rel);
    }
  }
  return out;
}
function read(rel){
  try{
    ...
    if(size>maxInspectBytes){skippedLargeFiles.push({file:rel,sizeBytes:size});return'';}
...
function evidence(files,patterns){
  const hits=[];
  for(const file of files){
    const content=contents.get(file)||'';
    if(!content)continue;
    for(const pattern of patterns){
      const flags=pattern instanceof RegExp?pattern.flags:'gi';
      const regex=pattern instanceof RegExp?new RegExp(pattern.source,flags.includes('g')?flags:flags+'g'):new RegExp(String(pattern),'gi');
      let match;let guard=0;
      while((match=regex.exec(content))&&guard++<30){
        const line=content.slice(0,match.index).split('\n').length;
        hits.push({file,line,match:String(match[0]).slice(0,180)});
...
function formInventory(file,content){
  const forms=[];const formRegex=/<form\b[\s\S]*?<\/form>/gi;let match;let index=0;
  while((match=formRegex.exec(content))){
    const html=match[0];
    if(!/type=["']email["']|name=["'](?:email|Email)["']/i.test(html))continue;
    forms.push({
      file,index:index++,id:html.match(/\bid=["']([^"']+)/i)?.[1]||null,name:html.match(/\bname=["']([^"']+)/i)?.[1]||null,
      action:html.match(/\baction=["']([^"']+)/i)?.[1]||null,method:(html.match(/\bmethod=["']([^"']+)/i)?.[1]||'script/default').toLowerCase(),
      dataNewsletter:/data-newsletter-form|data-newsletter-capture/i.test(html),
      explicitConsentControl:/type=["']checkbox["'][^>]*(?:consent|marketing)|name=["'](?:consent|marketingConsent|marketing_consent)["']/i.test(html),
      honeypot:/name=["']website["']|autocomplete=["']off["'][^>]*tabindex=["']-1["']/i.test(html),
      statusRegion:/aria-live|newsletter-status|form-status/i.test(html),snippet:html.replace(/\s+/g,' ').slice(0,240)
    });
  }
  return forms;
}
function anyContentIncludes(token){for(const content of contents.values())if(content&&content.includes(token))return true;return false;}
function anyContentMatches(regex){for(const content of contents.values()){if(!content)continue;regex.lastIndex=0;if(regex.test(content))return true;}return false;}
...
const requiredRoutes=policy.requiredRoutes.map(route=>{
  const escaped=route.replace(/[.*+?^${}()|[\]\\]/g,'\\$&');
  return{route,present:routes.includes(route)||anyContentIncludes(`'${route}'`)||anyContentIncludes(`"${route}"`),evidence:evidence(files,[new RegExp(escaped,'i')]).slice(0,10)};
});
const requiredObjects=policy.requiredDataObjects.map(object=>{
  const regex=new RegExp(`\\b${object.replace(/[.*+?^${}()|[\]\\]/g,'\\$&')}\\b`,'i');
  return{object,present:tables.includes(object.toLowerCase())||anyContentMatches(regex),tableDetected:tables.includes(object.toLowerCase()),evidence:evidence(files,[regex]).slice(0,10)};
});
const requiredSurfaces=policy.requiredSurfaces.map(surface=>{
  const patterns={newsletter_signup:/newsletter|membership/i,confirmation_result:/confirm|verified/i,preference_center:/preference/i,unsubscribe_result:/unsubscribe/i,subscriber_dashboard:/member-dashboard|subscriber-dashboard/i,admin_campaign_dashboard:/admin.*campaign|campaign.*admin/i};
  const pattern=patterns[surface]||new RegExp(surface.replaceAll('_','[-_ ]'),'i');
  const matches=emailPages.filter(file=>pattern.test(`${file} ${(contents.get(file)||'').slice(0,4000)}`));
  return{surface,present:matches.length>0,files:matches};
});

const staleCopy=[];
for(const file of emailPages){
  const content=contents.get(file)||'';
  if(/Cloudflare KV capture active/i.test(content))staleCopy.push({file,issue:'Public copy states KV capture is active although the Worker prefers D1 MEMBERS_DB and uses KV as compatibility fallback.'});
  if(/Unsubscribe link will be added/i.test(content))staleCopy.push({file,issue:'Public copy promises a future unsubscribe link rather than proving a current preference/unsubscribe lifecycle.'});
  if(/Weekly Signal Drop enabled/i.test(content)&&!stages.find(item=>item.stage==='weekly_campaign_build').strongEvidence.length)staleCopy.push({file,issue:'Signup success copy says weekly delivery is enabled although no verified campaign lifecycle was detected.'});
}
const consentRisks=[];
for(const form of emailForms)if(!form.explicitConsentControl)consentRisks.push({file:form.file,id:form.id,issue:'No explicit marketing-consent control detected; newsletter.js supplies a required consent checkbox before submission.'});
const providerRisks=[];
...
const uniqueSkippedLargeFiles=[...new Map(skippedLargeFiles.map(item=>[item.file,item])).values()].sort((a,b)=>a.file.localeCompare(b.file));
const summary={filesInspected:files.length,filesSkippedAsOversized:uniqueSkippedLargeFiles.length,emailPages:emailPages.length,emailForms:emailForms.length,formsWithoutStaticExplicitConsent:consentRisks.length,routesDetected:routes.length,databaseObjectsDetected:tables.length,environmentBindingsDetected:envBindings.length,scheduledEmailWorkflows:scheduledEmailWorkflows.length,stageStatus:statusCounts,implementedStages:implemented.length,partialStages:partial.length,missingStages:missing.length,activationBlockers:blockers.length,stalePublicCopyFindings:staleCopy.length,providerRiskFindings:providerRisks.length};
const report={ok:true,mode:'audit-only',version:policy.version,generatedAt:sourceCheckpoint(),liveEmailSend:false,providerContactMutation:false,campaignCreation:false,campaignScheduling:false,subscriberMutation:false,d1MigrationExecution:false,workerMutation:false,paymentActivation:false,boundary:'This inventory reads bounded repository text files only. It does not call Brevo, execute D1 migrations, mutate subscribers, send email, schedule campaigns or change Worker routes.',summary,stages,requiredRoutes,requiredObjects,requiredSurfaces,forms:emailForms,routes,tables,envBindings,emailPages,sqlFiles,workflowFiles,scheduledEmailWorkflows,skippedLargeFiles:uniqueSkippedLargeFiles,risks:{stalePublicCopy:staleCopy,consent:consentRisks,provider:providerRisks},activationBlockers:blockers};
...
console.log(`PHASE 4 EMAIL INVENTORY: ${implemented.length} implemented; ${partial.length} partial; ${missing.length} missing; ${blockers.length} blockers; ${uniqueSkippedLargeFiles.length} oversized machine file(s) skipped.`);
