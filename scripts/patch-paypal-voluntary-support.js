...
if (!source.includes('async function donationConfig(request,env)')) {
  const bootstrapAnchor = 'async function bootstrapPlans(request,env)';
  const bootstrapIndex = source.indexOf(bootstrapAnchor);
  if (bootstrapIndex < 0) throw new Error('PayPal bootstrap function insertion target not found');
  source = `${source.slice(0, bootstrapIndex)}${donationFunctions}\n${source.slice(bootstrapIndex)}`;
  changed = true;
} else {
  const replacements = [
