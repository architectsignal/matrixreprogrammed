...
try {
  ...
  for (const relative of surfaces) {
    ...
    assert(digest(relative) === sourceHash, `${relative} is not byte-identical to the canonical source.`);
    assert(count(html, /<h1\b/gi) === 1, `${relative} does not contain exactly one H1.`);
    assert(count(html, /<section\b[^>]*class=["'][^"']*\bhero\b[^"']*["']/gi) === 1, `${relative} does not contain exactly one hero.`);
    assert(count(html, /<!--\s*black-file-public-hero:start\s*-->/gi) === 1, `${relative} lacks one start marker.`);
    assert(count(html, /<!--\s*black-file-public-hero:end\s*-->/gi) === 1, `${relative} lacks one end marker.`);
    assert(/<h1>THE BLACK FILE<\/h1>/i.test(html), `${relative} lacks the canonical H1.`);
    assert(/id=["']black-file-public-lead["']/i.test(html), `${relative} lacks the canonical lead marker.`);
    assert(/href=["']#request["']/i.test(html), `${relative} lacks the request CTA.`);
    assert(/id=["']request["']/i.test(html), `${relative} lost the request target.`);
    assert(html.includes('downstream-records'), `${relative} lost downstream archive content.`);
  }
  assert(read('black-file.html').includes('matrix-pathways-boundary'), 'Source pathway content was removed.');
  assert(read('black-file.html').includes('Preserve downstream evidence and archive content.'), 'Source evidence content was removed.');
  assert(read('unrelated.html') === untouched, 'An unrelated page was modified.');

  const firstHashes = Object.fromEntries([...surfaces, 'unrelated.html'].map(relative => [relative, digest(relative)]));
  runFinalizer();
  for (const [relative, expected] of Object.entries(firstHashes)) {
    assert(digest(relative) === expected, `${relative} changed across repeated finalization.`);
  }
  ...
  assert(report.surfaces.length === 3, `Expected three finalized surfaces; found ${report.surfaces.length}.`);
  assert(report.surfaces.every(surface => surface.sha256 === report.sourceSha256), 'Report contains surface hash drift.');

  console.log('BLACK FILE PUBLIC HERO TEST PASSED');
  console.log('One canonical H1 is preserved across source and deployable aliases without losing request, pathway or downstream archive content.');
} finally {
