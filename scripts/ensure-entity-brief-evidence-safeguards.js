...
const block = `${start}<section id="entity-evidence-gap-safeguard" class="section wrap" style="display:block;content-visibility:visible!important;contain:none!important"><div class="eyebrow">Evidence Gap / Next Verification</div><h2>Missing Records</h2><p>Open and verify the primary page, complete filing, PDF, docket, contract, award notice, ownership record or registry entry before upgrading the assessment. Record provenance, publication date, completeness, redactions and any contradictory source.</p><h2>Watch Next</h2><p>Monitor new official releases, court filings, corrections, contracts, ownership records and authenticated correspondence. Reassess the conclusion whenever evidence confirms, weakens or replaces the current inference.</p><p><strong>Evidence boundary:</strong> A missing record is a research requirement, not proof of concealment, guilt, coordination or intent.</p></section>${end}`;
...
for (const name of fs.readdirSync(directory).filter(name => name.endsWith('.html'))) {
  ...
  if (!/<h2>Missing Records<\/h2>/i.test(html) || !/<h2>Watch Next<\/h2>/i.test(html) || !/Evidence boundary:/i.test(html)) {
