...
if(exists('_site'))for(const relative of ['index.html','public-consequence-contracts.html','accountability-tracker-runtime.js','accountability-tracker-runtime.css','member-dashboard.html','member-dashboard-app.js'])if(!exists(relative,outputRoot))fail(`Cloudflare output missing ${relative}`);
const report={ok:failures.length===0,generatedAt:new Date().toISOString(),failures,warnings,checks:{boundedDailyScheduler:failures.every(item=>!item.includes('Bounded tracker')&&!item.includes('cron')),persistentVersionHistory:failures.every(item=>!item.includes('contract_versions')),followedDashboardEvents:failures.every(item=>!item.includes('member_entity_follows')&&!item.includes('Member dashboard')),classifiedEvidence:failures.every(item=>!item.includes('Evidence layer')),editorialVerdictGate:failures.every(item=>!item.includes('accepted')&&!item.includes('reviewer')&&!item.includes('terms')),publicLiveRuntime:failures.every(item=>!item.includes('runtime'))}};
