...
const layerPanelsPath = path.join(root, 'data', 'atlas-layer-panels.json');
if (!fs.existsSync(atlasPath)) {
  console.log('No data/power-atlas.json found. Skipping Phase 2 Power Atlas build.');
  process.exit(0);
}
const atlas = JSON.parse(fs.readFileSync(atlasPath, 'utf8'));
const atlasLayerPanels = fs.existsSync(layerPanelsPath) ? JSON.parse(fs.readFileSync(layerPanelsPath, 'utf8')) : { panels: {} };
...
function nav() {
  return `<header class="wrap topbar"><a class="brand" href="index.html"><img src="sigil.png" alt="Matrix Reprogrammed sigil" /> MATRIX REPROGRAMMED</a><nav class="nav"><a href="index.html">Home</a><a href="start-here.html">Start Here</a><a href="power-atlas.html">Power Atlas</a><a href="control-system-tracker.html">Control Tracker</a><a href="atlas-layers.html">Atlas Layers</a><a href="atlas-index.html">Atlas Nodes</a><a href="evidence-vault.html">Evidence Vault</a><a href="books.html">Books</a><a href="news.html">Intel Desk</a><a href="search.html">Search</a><a href="black-file.html">Black File</a></nav></header>`;
}
...
function laneMiniCard(lane = {}) {
  return `<article class="card"><span class="label">Current lane</span><h3>${esc(lane.label || 'Lane')}</h3><p><strong>Examples:</strong> ${esc(lane.examples || '')}</p><p><strong>Sources:</strong> ${esc(lane.source || '')}</p><p><strong>Update:</strong> ${esc(lane.update || 'Weekly')}</p></article>`;
}
function powerAtlasLayerPanelsSection() {
  const panelEntries = Object.entries(atlasLayerPanels.panels || {});
  if (!panelEntries.length) return '';
  const cards = panelEntries.map(([slug, panel]) => {
    const categories = pillList(panel.categories || []);
    const firstLanes = (panel.lanes || []).slice(0, 4).map(laneMiniCard).join('');
    return `<section id="power-atlas-expanded-${esc(slug)}" class="section wrap"><div class="split"><div><span class="label">Atlas Layer Expanded</span><h2>${esc(panel.title || slug)}</h2><p class="lead">${esc(panel.summary || '')}</p><p>${categories}</p></div><aside class="card redline"><h3>Update Discipline</h3><p>Each lane needs a source, a date checked, an evidence class, and a category boundary before claims are treated as settled.</p><a class="btn alt" href="atlas-layers.html#layer-${esc(slug)}">Open Full Layer</a></aside></div><div class="grid">${firstLanes}</div></section>`;
  }).join('');
  return `<section id="power-atlas-expanded-layer-map" class="section wrap"><h2>Atlas Layers — Expanded Current Lanes</h2><p class="lead">This is the visible working layer map for current entity pages. It tracks what belongs in People, Institutions, Operations, Money Flows, Legal Records, Symbolic Layer, and Human Cost.</p><div class="cta-row"><a class="btn" href="atlas-layers.html">Open Full Atlas Layers</a><a class="btn alt" href="control-system-tracker.html">Control System Tracker</a><a class="btn alt machine-data-link" href="data/atlas-layer-panels.json">Layer Panel Data</a></div></section>${cards}`;
}
...
function patchPowerAtlas() {
  ...
  const section = `<section id="phase-two-atlas-engine" class="section wrap"><h2>Phase 2 Power Atlas Nodes</h2><p class="lead">Generated atlas nodes now give every major power lane a source boundary, evidence class, relationship-line language, and reader route.</p><div class="cta-row"><a class="btn" href="atlas-index.html">Open Full Atlas Index</a><a class="btn alt" href="control-system-tracker.html">Control System Tracker</a><a class="btn alt" href="atlas-layers.html">Atlas Layers</a><a class="btn alt" href="network-maps.html">Relationship Rules</a><a class="btn alt" href="evidence-policy.html">Evidence Policy</a></div><div class="grid">${nodeCards}</div></section>`;
  const expanded = powerAtlasLayerPanelsSection();
  if (!html.includes('id="phase-two-atlas-engine"')) {
    html = html.replace('</main>', `${section}${expanded}</main>`);
  } else if (!html.includes('id="power-atlas-expanded-layer-map"')) {
    html = html.replace('</main>', `${expanded}</main>`);
  }
...
const indexBody = `<main><section class="hero wrap"><div class="eyebrow">Phase 2 Power Atlas</div><h1>ATLAS NODES.</h1><p class="lead">A generated index of public-record nodes for institutions, networks, contractors, cases, crime systems, symbolic systems, war machinery, medical power, and gateway maps.</p><div class="cta-row"><a class="btn" href="power-atlas.html">Power Atlas</a><a class="btn alt" href="control-system-tracker.html">Control Tracker</a><a class="btn alt" href="atlas-layers.html">Atlas Layers</a><a class="btn alt" href="evidence-vault.html">Evidence Vault</a><a class="btn alt" href="network-maps.html">Network Maps</a><a class="btn alt" href="search.html">Search Archive</a></div></section><section class="section wrap split"><div class="terminal">ATLAS ENGINE STATUS\n&gt; Nodes: ${atlas.nodes.length}\n&gt; Evidence classes: ${atlas.evidenceClasses.length}\n&gt; Relationship types: ${atlas.relationshipTypes.length}\n&gt; Generated pages: ${generatedFiles.length}\n&gt; Search index: expanded\n&gt; Sitemap: expanded\n&gt; Control tracker: linked</div><aside class="card redline"><h2>Line Discipline</h2><p>Every relationship must say what it is. Worked for, funded, met, named in court, alleged, disputed, and symbolic similarity are not the same thing.</p></aside></section>${typeSections}</main>`;
...
patchPowerAtlas();
patchSearchIndex();
require('./build-policy-convergence-tracker.js');
generatedFiles.push('control-system-tracker.html');
addPagesToSitemap(generatedFiles);
patchLlms(generatedFiles);
console.log(`Built Phase 2 Power Atlas with ${atlas.nodes.length} nodes and ${generatedFiles.length} generated pages, plus expanded layer panels and control-system tracker integration.`);
