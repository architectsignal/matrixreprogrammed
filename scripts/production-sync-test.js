...
check('forum Worker entrypoint active', read('wrangler.toml').includes('main = "src/worker-forum-persistence.js"') && read('wrangler.jsonc').includes('"main": "src/worker-forum-persistence.js"'));
...
const report = { ok: failures.length === 0, generatedAt: new Date().toISOString(), failures, forumPersistence: 'D1 authoritative with KV recovery mirror and live post-deploy write/read proof.' };
...
console.log('Production synchronization assurance passed with D1 forum persistence.');
