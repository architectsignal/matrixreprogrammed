...
for (const [name, group] of Object.entries(Object.groupBy ? Object.groupBy(arr(recordEvents.events), e => slug(arr(e.entity_names)[0] || arr(e.institution_names)[0] || 'unknown')) : {})) {
  if (!group || group.length < 2) continue;
  const grades = uniq(group.map(e => e.evidence_grade));
  const lanes = uniq(group.map(e => e.source_lane));
  if (grades.length > 2 || lanes.length > 2) contradictions.push({ id:name, entity: arr(group[0].entity_names)[0] || arr(group[0].institution_names)[0] || name, issue:'Multiple grades or source lanes require human review.', grades, lanes, records: group.slice(0,5).map(e=>({title:e.summary,url:e.source_url,grade:e.evidence_grade})) });
}
...
function page(title, lead, cards, buttons=''){ return `<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"/><meta name="viewport" content="width=device-width, initial-scale=1.0"/><title>${esc(title)} | Matrix Reprogrammed</title><meta name="description" content="${esc(lead)}"/><link rel="stylesheet" href="styles.css"/><link rel="stylesheet" href="fixes.css"/></head><body><canvas id="matrix"></canvas><div class="signal-face"></div><div class="veil"></div><div class="page"><header class="wrap topbar"><a class="brand" href="index.html"><img src="sigil.png" alt="Matrix Reprogrammed sigil"/> MATRIX REPROGRAMMED</a><nav class="nav"><a href="daily-command-brief.html">Command Brief</a><a href="entity-daily-briefs.html">Entity Briefs</a><a href="private-contractor-tracker.html">Contractors</a><a href="search.html">Search</a></nav></header><main><section class="hero wrap"><div class="eyebrow">Master Brief Engine</div><h1>${esc(title).toUpperCase()}</h1><p class="lead">${esc(lead)}</p><div class="cta-row">${buttons}</div></section><section class="section wrap"><div class="grid">${cards}</div></section></main><footer class="footer wrap"><p><strong>Boundary:</strong> record grade first, conclusion second.</p></footer></div><script src="matrix.js"></script><script src="analytics.js"></script></body></html>`; }
...
function detailPage(folder, item, body){ write(`${folder}/${item.id}.html`, `<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"/><meta name="viewport" content="width=device-width, initial-scale=1.0"/><title>${esc(item.name)} | Matrix Reprogrammed</title><link rel="stylesheet" href="../styles.css"/><link rel="stylesheet" href="../fixes.css"/></head><body><main class="wrap"><h1>${esc(item.name)}</h1>${body}<p><a class="btn" href="../daily-command-brief.html">Daily Command Brief</a></p><p><strong>Boundary:</strong> public-record tracking, not a conclusion of intent.</p></main></body></html>`); }
for (const p of billionaireProfiles) detailPage('billionaire-briefs', p, `<p>Control score: ${p.control_score}. Level: ${esc(p.level)}.</p><p>Layers: ${esc(p.control_layers.join(', '))}</p><p>Ecosystems: ${esc(p.ecosystems.join(', '))}</p><h2>Missing Records</h2><pre>${esc(mdList(p.missing_records))}</pre>`);
for (const p of institutionProfiles) detailPage('institution-briefs', p, `<p>Control score: ${p.control_score}. Level: ${esc(p.level)}.</p><p>Routes: ${esc(p.routes.join(', '))}</p><h2>Missing Records</h2><pre>${esc(mdList(p.missing_records))}</pre>`);
for (const p of subjectProfiles) detailPage('subject-briefs', p, `<p>Layers: ${esc(p.control_layers.join(', '))}</p><h2>Top Entities</h2><pre>${esc(mdList(p.top_entities))}</pre><h2>Missing Records</h2><pre>${esc(mdList(p.missing_records))}</pre>`);
for (const p of [...mainPlayers.values()].slice(0,120)) detailPage('main-players', p, `<p>Roles: ${esc(p.roles.join(', '))}</p><p>Companies: ${esc(p.companies.join(', '))}</p><h2>Watch Next</h2><pre>${esc(mdList(p.watch_next))}</pre>`);
for (const t of timelines) detailPage('entity-timelines', t, `<h2>Timeline</h2><pre>${esc(mdList(t.events.map(e=>`${e.date || updated}: ${e.title} (${e.grade || 'grade pending'})`)))}</pre>`);
...
console.log('Master Brief Engine built: command, quality, missing records, billionaires, institutions, subjects, contradictions, main players and timelines.');
