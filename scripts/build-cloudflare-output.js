...
runRequired('Open-source PDF evidence reader build','scripts/build-evidence-reader.js');
runRequired('Open-source evidence timeline build','scripts/build-evidence-timeline.js');
runRequired('Phase 8 evidence archive build','scripts/build-evidence-archive.js');
...
runRequired('Open-source research suite test','scripts/open-source-research-suite-test.js');
...
const privateArchivePath=path.join(out,'evidence-archive');if(fs.existsSync(privateArchivePath)&&fs.statSync(privateArchivePath).isDirectory()){console.error('Cloudflare output failed: private evidence-archive directory exposed.');process.exit(1)}
for(const privatePath of ['data/source-snapshots','data/source-snapshot-index.json','data/source-change-ledger.json','downloads/source-change-monitor-report.json','downloads/source-change-preservation-hardening-report.json','downloads/search-v3-build-report.json','downloads/search-v3-runtime-report.json','downloads/search-v3-quality-test.json','downloads/evidence-network-map-build.json','downloads/evidence-network-map-wiring.json','downloads/public-network-map-test.json','downloads/osint-worker-patch-report.json','downloads/osint-tools-test.json','downloads/research-tools-ui-patch.json','downloads/market-activity-test.json','downloads/phase6-data-integration.json','downloads/phase6-worker-patch.json','downloads/phase6-integration-report.json','downloads/sec-market-activity-collection-report.json','downloads/open-source-research-suite-test.json','downloads/open-source-research-wiring.json','downloads/pagefind-output-test.json','downloads/phase8-evidence-archive-build.json','downloads/phase8-evidence-archive-test.json','downloads/phase8-wiring.json','downloads/browsertrix-crawl-plan.json','browsertrix-output','tools']){if(fs.existsSync(path.join(out,privatePath))){console.error(`Cloudflare output failed: private evidence archive, runner code or diagnostics exposed at _site/${privatePath}`);process.exit(1)}}
...
console.log(`Cloudflare output ready: ${count.length} deployable files, including Phase 8 WACZ replay, signed integrity manifests, visual source diffs, Citation.js exports, Phase 6 market activity, PDF.js evidence reading, vis-timeline chronology, Pagefind fallback wiring, Search V3 and evidence-led network maps while private diagnostics remain excluded.`);
