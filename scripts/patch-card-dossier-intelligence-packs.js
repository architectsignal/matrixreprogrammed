...
const deckDataCache={};
function readDeckData(deck){if(deckDataCache[deck.id])return deckDataCache[deck.id];const file=deck.source||`data/${deck.id}-deck.json`;const data=js(file,{cards:[],deck:[]});const arr=data.cards||data.deck||[];const map=new Map(arr.map(c=>[slug(c.id||c.name||c[1]),c]));deckDataCache[deck.id]={data,map};return deckDataCache[deck.id];}
function val(obj,keys,fallback=''){for(const k of keys){if(obj&&obj[k]!==undefined&&obj[k]!==null&&obj[k]!=='')return obj[k];}return fallback;}
function list(items){const arr=Array.isArray(items)?items:String(items||'').split(/[;,]/).map(x=>x.trim()).filter(Boolean);return arr.length?`<ul>${arr.slice(0,12).map(x=>`<li>${esc(x)}</li>`).join('')}</ul>`:'<p>Needs source-backed entries.</p>'}
const profiles={
...
function deepSummary(deckId,card,raw,feedCard){const p=profiles[deckId]||profiles['institutions'];const name=card.name||val(raw,['name'],card.id);const lane=card.lane||val(raw,['lane','role','history'],'research lane');const history=val(raw,['history','subtitle','base'],lane);const score=card.score||val(raw,['score','powerScore'],feedCard?.score||'');const confidence=feedCard?.scoreModel?.confidence||'watch';const why=val(raw,['why'],feedCard?.speculativeRationale||`${name} is included as a source-route card inside ${deckId}.`);const sourceRoute=val(raw,['sourceRoute'],`Build this card from official pages, public records, archives, filings, regulator documents and reliable reporting about ${name}.`);return{p,name,lane,history,score,confidence,why,sourceRoute};}
function pack(deck,card){const {map}=readDeckData(deck);const raw=map.get(card.id)||{};const feedCard=feedMap.get(`${deck.id}:${card.id}`)||{};const d=deepSummary(deck.id,card,raw,feedCard);const extra=[];
...
let checked=0,patched=0,missing=[];
for(const deck of manifest.decks||[]){for(const card of deck.cards||[]){checked++;const route=String(card.profileRoute||'').split('?')[0];if(!route||!ex(route)){missing.push({deck:deck.id,card:card.id,route});continue;}let html=rd(route);const before=html;html=replacePack(html,pack(deck,card));if(html!==before)patched++;wr(route,html);}}
const report={ok:missing.length===0,updated:new Date().toISOString(),checkedCards:checked,patchedOrRefreshed:patched,missing};
...
console.log(`Card dossier intelligence packs complete: ${checked} checked, ${patched} patched, ${missing.length} missing.`);
