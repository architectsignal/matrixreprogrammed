...
for (const marker of ["import {", 'handlePublicInvestigationRoute', 'isPublicInvestigationRoute(path)']) needText('src/worker-production.js', marker, `public investigation route marker ${marker}`);
for (const marker of ['/api/investigate','matrix_public_investigations','validatePublicInvestigationResult','MATRIX_PUBLIC_INVESTIGATION_LOCAL_ENRICHMENT_ENABLED','MATRIX_PUBLIC_INVESTIGATION_FRESH_SOURCES_ENABLED','OfficialFreshSourceDirector','matrix_public_investigation_proofs','no-eligible-model-evidence-fallback','fallback_used']) needText('src/worker-public-investigation.js', marker, `public investigation boundary ${marker}`);
...
for (const marker of ['/api/ai-management/admin/bounty-engine','runBountyCompletionCycle','ENGINE OPERATIONAL / FIRST RECEIPT PENDING','automatic_claims: false','security_execution: false']) needText('src/worker-bounty-engine.js', marker, `Bounty engine boundary ${marker}`);
...
for (const marker of ['CAUSE NO HARM OR LOSS.','matrix_constitution_immutable_update','CREATE TABLE IF NOT EXISTS matrix_operating_missions','CREATE TABLE IF NOT EXISTS matrix_capability_snapshots','CREATE TABLE IF NOT EXISTS matrix_delegations']) needText('migrations/phase17_matrix_operating_system.sql', marker, `Matrix operating-system migration marker ${marker}`);
for (const marker of ['CREATE TABLE IF NOT EXISTS matrix_capability_graph','CREATE TABLE IF NOT EXISTS matrix_human_dependencies','CREATE TABLE IF NOT EXISTS matrix_site_health_checks','CREATE TABLE IF NOT EXISTS matrix_acceptance_receipts']) needText('migrations/phase18_matrix_continuous_evolution.sql', marker, `Matrix continuous-evolution migration marker ${marker}`);
