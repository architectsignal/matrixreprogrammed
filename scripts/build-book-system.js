...
function nav() {
  return `<header class="wrap topbar"><a class="brand" href="index.html"><img src="sigil.png" alt="Matrix Reprogrammed sigil" /> MATRIX REPROGRAMMED</a><nav class="nav"><a href="index.html">Home</a><a href="start-here.html">Start Here</a><a href="books.html">Books</a><a href="search.html">Search</a><a href="news.html">Intel Desk</a><a href="forum.html">Signal Board</a><a href="black-file.html">Black File</a><a href="transmissions.html">Rumble</a></nav></header>`;
}
...
const startCards = [
  ['Secret societies / symbols', 'As Above, So Below, D.O.G, Masonic Symbols, Degree 1, Degree 3', ['dog-the-architect','masonic-symbols','degree-one','degree-three']],
...
const startBody = `<main><section class="hero wrap"><div class="eyebrow">Start Here</div><h1>CHOOSE YOUR DOOR.</h1><p class="lead">The archive is large by design. Pick the signal you are trying to understand and enter through the right reader path.</p><div class="cta-row"><a class="btn" href="black-file.html">Open The Black File</a><a class="btn alt" href="search.html">Search Archive</a><a class="btn alt" href="forum.html">Signal Board</a></div></section><section class="section wrap grid">${startCards.map(([title, desc, keys]) => `<article class="card"><h2>${esc(title)}</h2><p>${esc(desc)}</p>${keys.map(key => books.find(b => b.key === key)).filter(Boolean).map(b => `<p><a class="btn" href="${esc(urlFor(b))}">${esc(b.title)}</a></p>`).join('')}</article>`).join('')}</section></main>`;
...
console.log(`Built ${books.length} confirmed generated book pages, books.html, start-here.html, search.html, and search-index.json.`);
