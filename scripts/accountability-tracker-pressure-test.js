...
const required=['src/worker-consequence-tracker.js','src/worker-consequence-evidence.js','src/worker-production.js','scripts/install-consequence-evidence-worker.js','scripts/finalize-accountability-tracker-runtime.js','scripts/add-accountability-alerts-dashboard.js','scripts/accountability-tracker-pressure-test.js','accountability-tracker-runtime.js','accountability-tracker-runtime.css','accountability-review-inbox.html','accountability-review-inbox.js','member-dashboard.html','member-dashboard-app.js','wrangler.toml','data/public-consequence-contracts.json','data/public-consequence-due-index.json','public-consequence-contracts.html','index.html','downloads/consequence-evidence-worker-install.json','downloads/accountability-tracker-runtime-report.json','downloads/accountability-alerts-dashboard-report.json'];
for(const relative of required)if(!exists(relative))fail(`Missing ${relative}`);
for(const relative of ['src/worker-consequence-tracker.js','src/worker-consequence-evidence.js','src/worker-production.js','scripts/install-consequence-evidence-worker.js','scripts/finalize-accountability-tracker-runtime.js','scripts/add-accountability-alerts-dashboard.js','scripts/accountability-tracker-pressure-test.js','accountability-tracker-runtime.js','accountability-review-inbox.js','member-dashboard-app.js']){
...
if(!failures.length){
  ...
  const runtime=read('accountability-tracker-runtime.js');for(const marker of ['/api/public/consequence-contracts?limit=100','card.dataset.contractId','Live D1 record','static-fallback'])if(!runtime.includes(marker))fail(`Public tracker runtime missing ${marker}`);
...
if(exists('_site'))for(const relative of ['index.html','public-consequence-contracts.html','accountability-tracker-runtime.js','accountability-tracker-runtime.css','accountability-review-inbox.html','accountability-review-inbox.js','member-dashboard.html','member-dashboard-app.js'])if(!exists(relative,outputRoot))fail(`Cloudflare output missing ${relative}`);
const report={ok:failures.length===0,generatedAt:new Date().toISOString(),failures,warnings,checks:{boundedDailyScheduler:failures.every(item=>!item.includes('Bounded tracker')&&!item.includes('cron')),persistentVersionHistory:failures.every(item=>!item.includes('contract_versions')),followedDashboardEvents:failures.every(item=>!item.includes('member_entity_follows')&&!item.includes('Member dashboard')),classifiedEvidence:failures.every(item=>!item.includes('Evidence layer')),editorialVerdictGate:failures.every(item=>!item.includes('accepted')&&!item.includes('reviewer')&&!item.includes('terms')),humanReviewInbox:failures.every(item=>!item.includes('Human review')),publicLiveRuntime:failures.every(item=>!item.includes('runtime'))}};
