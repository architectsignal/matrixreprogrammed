...
if (!html.includes('id="epstein-people-tracker"')) {
  html = html.replace('</main>', `${peopleSection}</main>`);
}
...
console.log(`Enhanced Epstein Evidence Watch with ${(data.watchSources || []).length} source lanes, ${(data.bulletins || []).length} bulletins, ${(emailSignals.signals || []).length} email/network signals, ${(peopleIndex.people || []).length} people/entity cards, Evidence Boundary marker, and download outputs.`);