...
const homepageNormalizer = path.join(root, 'scripts', 'normalize-homepage-mission-copy.js');
if (!fs.existsSync(homepageNormalizer)) throw new Error('Canonical homepage normalizer is missing');
execFileSync(process.execPath, [homepageNormalizer], { cwd: root, stdio: 'inherit' });
...
function workerHasAlias(worker, route, target) {
  return worker.includes(`'${route}': '${target}'`) || worker.includes(`'${route}':'${target}'`) || worker.includes(`"${route}": "${target}"`) || worker.includes(`"${route}":"${target}"`);
}
function targetExists(target) {
  const clean = String(target || '').replace(/^\//, '');
  return Boolean(clean) && exists(clean);
}
...
const homepageMarker = 'MAP THE STRUCTURE. READ THE SIGNALS.';
...
const aliases = Object.entries(requiredAliasMap).map(([route, target]) => {
  const explicitAlias = workerHasAlias(worker, route, target);
  const extensionlessFallback = targetExists(target);
  return { route, target, present: explicitAlias || extensionlessFallback, explicitAlias, extensionlessFallback };
});

const modules = [
  { name: 'Homepage', file: 'index.html', marker: homepageMarker },
...
const hardBoardRouteChecks = hardBoardRoutes.map(route => ({ route, present: worker.includes(route) }));
const status = {
  ...
  liveProof: {
    homepageExpectedMarker: homepageMarker,
    ...
    cacheNote: 'The deployed commit must match this build SHA before live verification is accepted.'
  }
};
...
const aliasRows = aliases.map(item => `<tr><td>${esc(item.route)}</td><td>${esc(item.target)}</td><td>${item.present ? 'Present' : 'Missing'}${item.extensionlessFallback ? ' / file route' : ''}</td></tr>`).join('');
